...
- The non-home menu trigger now centers its current-route art and `∏ﬁ¥∫` / `Menu` label as a single group instead of pinning the art against the left edge of a wide bar.
...
- The scene stays text-light: one large lower-right `«Æ±‚` / `Play` action plus destination-only buttons around Pip, with all labels preserved for assistive technology.
...
- Corrected the focused-play back label to Puzzle Room / ?ºÏ¶êÎ∞? matching its existing route instead of falsely calling it the Picture List.
...
- Renamed the Korean player-facing space to `Pip??Í∑∏Î¶ºÎ∞?, retaining the English product name only where it is part of the brand. The room, Pantry route, and home copy now use the same familiar Korean concept.
- Rebuilt the first landing screen as Pip's actual warm picture room: Pip, one clear current-picture action, and six direct destinations. The picture shelf only opens after `Í∑∏Î¶º Î™©Î°ù`, so the initial screen invites play rather than presenting a catalogue.
...
- Localized the Korean brand title to `Pip??Í∑∏Î¶º ?¨Ìä∏Î¶?, replaced Settings guide replay placeholders with approved raster art, centered Time Attack Start, and consolidated Quick Travel into the requested vertical order.
...
- The completion overlay has distinct final-shelf copy and action, so it never promises a nonexistent ?únext shelf.??- Verified focused shelf/save/i18n tests and mobile visual QA at 360x740, 390x844, 430x932, and 675x900. No Android bundle was created.
...
- Changed future paid-stage copy from vague Coming Soon wording to a price-preview placeholder (`$0.99 planned` / `?àÏÉÅ Í∞ÄÍ≤?$0.99`) for later store wiring.
...
- Replaced hardcoded `$0.99 planned` add-on text with store-safe value copy: `Optional add-on - 100 puzzles` / `?†ÌÉù Ï∂îÍ? ?∏Ìä∏ - 100Í∞??ºÏ¶ê`.
...
- Restored the music toggle in Settings as `Cozy music on/off` / `?ÑÎäë???åÏïÖ ÏºúÍ∏∞/?ÑÍ∏∞`; music remains opt-in and does not autoplay by default.
...
- Renamed the first-puzzle help heading from the Korean `Í∑∏Î¶º Ïπ?Í∞Ä?¥Îìú` wording to a friendlier picture-square guide label, while keeping the visual clue examples for adjacent 3 cells and separated 1 1 1 cells.
...
- Fixed the Korean floating menu label so it renders as `Î©îÎâ¥` instead of mojibake.
...
- ?Ä?ÑÏñ¥???Ä?ÑÏïÑ??Í≤∞Í≥º?êÏÑú Î≥¥ÏÉÅ???àÎäî ?Ä?ÑÏïÑ?ÉÍ≥º ÏßÑÌñâ Î∂ÄÏ°±ÏúºÎ°?Î≥¥ÏÉÅ???ÜÎäî ?Ä?ÑÏïÑ?ÉÏùÑ Î¨∏Íµ¨Î°?Î∂ÑÎ¶¨?àÎã§.
- Í≤∞Í≥º Ïπ¥Îìú???¨Ïö©???åÌä∏ ?òÎ? ?∏Ï∂ú?? Í∏∞Î°ù Í≤ΩÏüÅÍ≥??§Ìëº ?åÎπÑÍ∞Ä ???îÎ©¥?êÏÑú ?®Íªò ?¥Ìï¥?òÎèÑÎ°??ïÎ¶¨?àÎã§.
- Î≤ÑÏ†Ñ ?úÍ∏∞??v0.1.248Î°?Í∞±Ïã†?àÎã§.

### v0.1.249 - Time Attack Record Hint Visibility
- ?Ä?ÑÏñ¥??ÏµúÍ≥† Í∏∞Î°ù ?îÏïΩÍ≥?Í∏∞Î°ù Î™©Î°ù???¨Ïö©???åÌä∏ ?òÎ? ?®Íªò ?úÏãú?òÎèÑÎ°??ïÎ¶¨?àÎã§.
- Season 0??333Í∞??ºÏ¶ê???®Ïàú Î™©Î°ù???ÑÎãà???§Ìëº ?òÍ∏âÍ≥?Pantry ÏßÑÌñâ?ºÎ°ú ?®Í≥Ñ?ÅÏúºÎ°??¥Ïñ¥ Í∞Ä??Íµ¨Ï°∞Î°??êÎÅºÍ≤??òÎäî UI Î≥¥Í∞ï?¥Îã§.
...
- The player-facing copy remains one gentle support/spoon surface; the guard fails if support-pack UI copy reintroduces paid/free or ?†Î£å/Î¨¥Î£å wording.
...
- The Billing QA forbidden-copy regex is already normalized to explicit Korean terms (`?†Î£å` / `Î¨¥Î£å`), so the guard remains readable and does not depend on mojibake fragments.
...
- Added a small Open/?¥Í∏∞ capsule to the opening Puzzle/Pantry/Time Attack chips so the launch promises read as tappable entry points without changing routing or the player-name pending-view flow.
...
- Localized human resident names in Korean story scenes as `Î∞??†ÏÉù??, `Î¶¥Î¶¨`, and `ÎßàÌÖå??; retained `Pip` as the established character and product brand name.
...
- Replaced the formal `Î∞??†ÏÉù?? localization with the warmer role-based name `?úÍ≥Ñ ?†ÏïÑÎ≤ÑÏ?` in his Korean title and first introduction.
- Uses the shorter `?†ÏïÑÎ≤ÑÏ?` after the introduction so repeated sentences sound natural while the clock remains his memorable first-encounter identity.
...
- Replaced the English `Jay` name-field example with the natural Korean example `?òÎäò` when the app is displayed in Korean.
- Replaced the fallback player address `Friend` with `ÏπúÍµ¨`, so Pip does not switch back to an English human name when a Korean player has no saved display name.
...
- Localized the recurring Sunny Spoon Sign and Apron Drawer picture names used by the first 40 post-starter puzzles, including natural `??Î≤àÏß∏ ...` variants, and localized their Korean stage/badge names.
...
- Added natural Korean titles for all ten motifs and their `??Î≤àÏß∏ ...` variants; changed the technically valid but awkward `?§Ìå®` label to the clearer player-facing `?§Ì???.
...
- Moved the Support Pack and Small Spoon Jar into the Pantry shop itself. Spoon-priced decoration filters, cards, and the ?úshow more??action always appear first; paid packs follow as the final shop section.
...
- Visual review confirms the Pantry order is room and request, spoon-priced decorations, ?úshow more,??then Support Pack and Small Spoon Jar. The full candidate gate and 51-frame v0.1.540 visual pack pass.
...
- Shortened the two shop section titles and renamed the ambiguous Korean starter item from Ï≤?Ïπ¥Ïö¥??Î≥?to Ï≤¥ÌÅ¨ Ïπ¥Ïö¥?∞Î≥¥.
...
- Removed the repeated Î∂Ä?ÅÌïú ?åÌíà target chip from the first Pantry request card; the artwork, title, and action already identify the item.
...
- Shortened the Korean and English labels to `?§Ïùå ?åÌíà` / `Next decoration` and concise item actions.
...
- Removed the secondary `Not now` / `?òÏ§ë??Î≥ºÍ≤å?? action from Pip conversations; story and tutorial moments now present one clear next action.
...
- Changed the final tutorial action from the awkward `Í∞ôÏù¥ ?úÏûë` / `Start together` to `?? Í∞ÄÎ≥¥Ïûê!` / `Ready? Go!`.
...
- Replaced the lingering Pantry ?úshow more decorations??glare card with one flat, full-width action and removed its retired meter, sparkle, and pseudo-element rules.
...
- Moved the spoon balance into the scene and rebuilt the current-puzzle call to action as a direct `???ºÏ¶ê ?ÄÍ∏? action with puzzle art and the active picture name.
