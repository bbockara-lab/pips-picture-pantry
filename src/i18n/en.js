import { SUMMER_PUZZLES } from "../data/summerPuzzles.js";

const summerPuzzleCopy = Object.fromEntries(SUMMER_PUZZLES.map((puzzle) => [
  puzzle.id,
  { title: puzzle.title, imageName: puzzle.title }
]));

export const en = {
  seasonalGift: {
    eyebrow: "A Korean Chuseok gift from Pip",
    title: "A warm harvest gift for you",
    body: "Pip saved a little Chuseok thank-you for everyone joining the Pantry around the harvest moon.",
    reward: "+{count} bonus spoons",
    continue: "Celebrate with Pip",
    artAlt: "Pip presenting a Chuseok harvest gift"
  },
  app: {
    title: "Pip's Picture Pantry",
    studioName: "Sunny Spoon Studios",
  },
  brandIntro: {
    ariaLabel: "Sunny Spoon Studios and Pip's Picture Pantry opening screens",
    skip: "Start"
  },
  playerIntro: {
    title: "What should Pip call you?",
    label: "Player name",
    placeholder: "Jay",
    continue: "Continue",
    defaultName: "Friend"
  },
  spoonRun: {
    eyebrow: "Spoon runs",
    intro: "Take today's picture trail, then revisit a favorite for a clean-run bonus.",
    availableToday: "spoons available today",
    homeOpportunity: "+{count} spoons available today",
    dailyReward: "+{count} spoons with today's bonus",
    replayReward: "+{count} spoons still available from clean replays",
    collected: "Today's reward collected",
    replayEmpty: "Finish any picture to open tomorrow's cozy replay picks."
  },
  views: {
    puzzle: "Pip's Puzzle Room",
    spoonRun: "Collect Spoons",
    album: "Album",
    pantry: "Pip's Pantry",
    map: "Badges",
    timeAttack: "Time Attack",
    mailbox: "Pip's Mailbox",
    navLabel: "Main views",
    menu: "Menu"
  },
  mailbox: {
    eyebrow: "Letters from the Pantry",
    title: "Pip's Mailbox",
    intro: "Developer letters and Pip's stories are kept here for you.",
    filters: { all: "All", letters: "Letters", stories: "Pip's Stories" },
    unreadCount: "{count} unread",
    welcomeTitle: "A warm hello from the developers",
    welcomePreview: "Thank you for visiting Pip's little Pantry.",
    welcomeDate: "August 2026 · Sunny Spoon Studios",
    welcomeArtAlt: "Pip writing a letter at a warm Pantry desk",
    welcomeBody: "Hello from Sunny Spoon Studios.\n\nThank you so much for finding Pip's Picture Pantry. We are carefully polishing this little puzzle room, one square at a time, into a warm place you will want to visit for a long while.\n\nWe will keep bringing you new picture puzzles, seasonal stories, and more comfortable ways to play. We hope you will look forward to every new scene with Pip.\n\nThe time you spend with Pip and your warm support mean more to us than you know.\n\n— Sunny Spoon Studios",
    koreanHarvest: {
      title: "A letter beneath the harvest moon",
      preview: "Pip prepared a warm Chuseok table for you.",
      date: "September 2026 · Sunny Spoon Studios",
      artAlt: "Hanbok Pip writing a letter beside songpyeon beneath the full moon",
      body: "Thank you for spending quiet puzzle moments with Pip.\n\nWe made this Korean harvest celebration with great care. Every moonlit picture, songpyeon plate, and little Pantry keepsake was prepared to share the warmth of Chuseok with players around the world.\n\nWe will keep filling the Pantry with new pictures, stories, and small surprises. We hope the full moon brings warmth to your day.\n\n— Sunny Spoon Studios"
    },
    messages: {
      "guide-puzzle": { title: "How picture puzzles work", preview: "Replay Pip's first puzzle guide." },
      "guide-cursor": { title: "Moving around bigger boards", preview: "Replay the D-pad guide." },
      "guide-pantry-jar": { title: "Pip's home display bonus", preview: "Review home displays and the extra-spoon chance." },
      "guide-time-attack": { title: "A note about Time Attack", preview: "Replay the quick challenge guide." },
      "guide-map": { title: "Pip's badge guide", preview: "See how your Pantry journey is recorded." },
      "story-spoon-run": { title: "Spoon Run", preview: "Pip explains where today's spoons are waiting." },
      "story-first-purchase": { title: "The first Pantry keepsake", preview: "Revisit Pip's first decoration story." },
      "story-pantry-room": { title: "A room coming to life", preview: "Revisit Pip's Pantry room story." },
      "story-mr-park": { title: "A visit from Grandpa Clock", preview: "Read this neighbor story again." },
      "story-lily": { title: "A visit from Lily", preview: "Read this neighbor story again." },
      "story-mateo": { title: "A visit from Mateo", preview: "Read this neighbor story again." }
    }
  },
  header: {
    settings: "Settings",
    resetProgress: "Reset progress",
    resetTitle: "Reset all progress?",
    resetBody: "This clears every saved picture and current puzzle board on this device.",
    cancelReset: "Keep playing",
    confirmReset: "Reset"
  },
  currency: {
    spoons: "Spoons {count}"
  },
  toast: {
    loginBonus: "+{count} daily spoons! 🥄"
  },
  settings: {
    title: "Settings",
    version: "Version {version} (build {build})",
    language: "Language",
    languageNote: "Use your device language by default, or choose a language for this app.",
    systemDefault: "System",
    english: "English",
    korean: "Korean",
    playerName: "Player name",
    savePlayer: "Use this name",
    sound: "Sound",
    sfx: "Effects on/off",
    music: "Cozy music on/off",
    controls: "Controls",
    controlsNote: "Choose how you move on bigger puzzles.",
    controlsAuto: "Auto for big boards",
    controlsDirect: "Tap cells",
    controlsCursor: "D-pad",
    cursorTrail: "Use Trail Paint",
    cursorOptions: "Additional Options",
    controlsDirectShort: "Tap Cells",
    controlsCursorShort: "Use D-pad",
    switchToDirect: "Switch to tapping cells",
    switchToCursor: "Switch to the D-pad",
    guideReplayTitle: "Pip's guide",
    guideReplayAction: "Show Pip's guide",
    guideReplayPuzzleAction: "Picture guide",
    guideReplayCursorAction: "D-pad guide",
    guideReplayPantryJarAction: "Home display bonus guide",
    guideReplayTimeAttackAction: "Time Attack guide",
    guideReplayMapAction: "Badge guide",
    spoonStoreTitle: "Add more spoons",
    supportTitle: "Pip Support Pack",
    supportBody: "Support Pip and add {spoons} spoons.",
    supportChecking: "Checking the store connection...",
    purchaseOpening: "Opening the {store} purchase window. Please wait...",
    purchaseOpeningButton: "Opening purchase...",
    supportAndroidOnly: "Store connection is being prepared.",
    supportReady: "Spoons arrive right after purchase.",
    supportCancelled: "Purchase was cancelled. You can try again anytime.",
    supportNetworkError: "The store connection is resting. Try again when the network is steady.",
    supportPendingConsumption: "Google Play is finishing the previous purchase. Try again shortly.",
    supportFailed: "The store could not finish that request. No spoons were spent.",
    supportFactSpoons: "{spoons} spoons",
    supportFactStore: "Google Play",
    supportFactAndroid: "Store preparing",
    supportFactRepeat: "Repeatable support",
    supportBuy: "Support for {price}",
    spoonJarTitle: "Small Spoon Jar",
    spoonJarBody: "Add {spoons} spoons when you need them.",
    spoonJarFactSpoons: "{spoons} spoons",
    spoonJarFactRepeat: "Repeatable top-up",
    spoonJarReady: "Spoons arrive after each jar.",
    spoonJarFailed: "The jar could not be filled. No spoons were spent.",
    spoonJarBuy: "{price} · {spoons} spoons",
    supportPricePending: "Store price",
    close: "Done"
  },
  badges: {
    collectionCount: "{earned}/{total} collected",
    groupA: "Shelf A - Pip’s First Steps",
    groupB: "Shelf B - Pip’s Bakery",
    groupC: "Shelf C - Pip’s Village",
    groupD: "Shelf D - Pip’s Seasons",
    groupE: "Shelf E - Pip’s Summer",
    groupF: "Shelf F - Harvest Moon Festival",
    earned: "Collected",
    stageRequirement: "Complete Stage {stage}",
    detailEarned: "This keepsake is shining on your shelf.",
    detailLocked: "Complete Stage {stage}: {completed}/{total} pictures finished.",
    featureOnHome: "Display on home",
    featuredOnHome: "Displayed on home",
    noFeatured: "Pick a badge to display",
    featuredAria: "{title}, displayed badge. Open badge shelf",
    toastLabel: "Badge earned!",
    pipBakeryDoor: "Bakery Door",
    pipPastryMorning: "Pastry Morning",
    pipTinCollection: "Tin Collection",
    pipVillagePath: "Village Path",
    pipClockCorner: "Clock Corner",
    pipFullPantry: "Full Pantry",
    pipPortrait: "Pip Portrait",
    summerPantry: "Summer Pantry Badge",
    summerPantryDesc: "Complete the summer Pantry picture collection.",
    koreanHarvest: "Harvest Moon Badge",
    koreanHarvestDesc: "Complete Pip's Korean harvest moon picture collection.",
    progress: "{completed}/{total} cards",
    earnedAria: "{title} badge earned",
    progressAria: "{title} badge progress {completed} of {total}",
    shelfEmptyHint: "Finish pictures to earn shelf badges.",
    shelfProgressHint: "{completed} pictures complete - {remaining} more for this badge!",
    earnedShelfAria: "{count} earned badges",
    pipsFirstShelf: "First Shelf Badge",
    pipsFirstShelfDesc: "Pip picture badge complete",
    sunnySpoonSign: "Sunny Spoon Sign Badge",
    sunnySpoonSignDesc: "Sunny Spoon logo badge complete",
    apronDrawer: "Apron Drawer Badge",
    apronDrawerDesc: "Pip helper badge complete",
    bakeryWindow: "Bakery Window Badge",
    bakeryWindowDesc: "Pantry friends badge complete",
    villagePantry: "Village Pantry Badge",
    villagePantryDesc: "Sunny Spoon cast badge complete"
  },

  sections: {
    starterShelf: "Starter shelf",
    dailyPicture: "Daily picture",
    currentPicture: "Current picture",
    startHere: "Start here",
    pantryAlbum: "Pantry Album",
    pantryRoom: "Pip's Pantry",
    pantryMap: "Badge Shelf"
  },
  home: {
    sceneAria: "Pip's Puzzle Room",
    summerEventWeek: "Summer Pantry Week",
    koreanHarvestEvent: "Korean Harvest Moon Festival",
    keepsakeShelfAria: "Home display shelf",
    destinationsAria: "Play destinations",
    currentPicture: "Ready to color",
    solveCurrent: "Solve this picture",
    pictureList: "Pictures",
    room: "Pip's room",
    greetingMessages: {
      0: "Ready for a picture?",
      1: "Let's make something cozy!",
      2: "One little cell at a time.",
      3: "What will we find today?",
      4: "Pip saved you a spot!",
      5: "Let's fill in the fun!",
      6: "A new picture is waiting!"
    },
    new: "New",
    collectionProgress: "{current} of {total} collected",
    pantryLabel: "Pip's Pantry",
    timeAttackLabel: "Time Attack",
    albumLabel: "Album",
    mapLabel: "Badges"
  },
  updatePolicy: {
    optionalMessage: "A new version {version} is ready! Want to visit the store?",
    mandatoryTitle: "A required update is ready",
    mandatoryMessage: "Please update to version {version} to keep playing safely.",
    updateNow: "Update",
    later: "Later"
  },
  playScreen: {
    aria: "Focused puzzle play",
    back: "Menu",
    open: "Play Now",
    hubNote: "Choose a picture, then open a clean screen just for solving."
  },
  daily: {
    eyebrow: "Today's picture",
    play: "Play",
    selected: "Selected",
    completed: "Completed today",
  },
  replayPicks: {
    title: "Replay picks",
    count: "{count}/{limit} today",
    review: "Review",
    challengeLabel: "Replay challenge",
    cleanRule: "No hints. No wrong fills. Finish clean for +1 spoon.",
    cleanBroken: "Bonus missed. Keep going for practice."
  },
  howToPlay: {
    title: "Pip's picture tip",
    pipLine: "I'll stay beside you. Fill the numbered groups, and safe blanks will tidy into X marks.",
    goal: "Color each group to match its clue.",
    clueTogether: "3 together",
    clueApart: "1 each, apart",
    stepFill: "Choose Color, then tap the squares that belong to the picture.",
    stepClues: "3 means three squares in a row. 1 1 means color one square, leave at least one empty square, then color one more.",
    stepMark: "When a square is surely empty, use Blank Check to mark it gently."
  },
  timeAttack: {
    title: "Time Attack",
    body: "Solve 5×5 → 8×8 → 10×10 in 3 minutes total. The clock keeps running between boards.",
    hubEyebrow: "Time Attack",
    hubTitle: "Three-puzzle challenge",
    hubAction: "Start",
    ladderAria: "Time Attack round sizes",
    ladderRound1: "Round 1",
    ladderRound2: "Round 2",
    ladderRound3: "Round 3",
    ladderSize1: "5x5",
    ladderSize2: "8x8",
    ladderSize3: "10x10",
    ladderWarmup: "Warm up fast and save spoons.",
    ladderTempo: "Bigger clues make hint timing matter.",
    ladderFinal: "Push for the record, then choose if a hint is worth it.",
    start: "Start",
    countdownEyebrow: "Get ready",
    countdownGo: "GO!",
    daily: "Daily rewards {count}/{limit}",
    records: "My records",
    recordsEmpty: "Finish all three rounds to leave your first record here.",
    recordLine: "{size}x{size} \u00b7 {progress} cells \u00b7 {time}",
    lastReward: "Complete! You earned +{reward} spoons.",
    lastNoReward: "Complete! Daily rewards are used up.",
    runPlanTitle: "Run plan",
    runPlanBody: "3 random boards. Hints can save a run, but each one spends spoons.",
    rewardTitle: "Daily rewards",
    rewardReady: "Today's spoons {count}/{limit}",
    rewardUsed: "Today's spoons collected",
    bestTitle: "Best run",
    newRecord: "New personal best",
    lastRun: "Last run",
    lastScore: "{progress} cells \u00b7 {time}",
    timeUp: "Time up",
    timeoutReward: "Time up \u00b7 +{reward} spoons",
    timeoutNoReward: "Reach halfway through the current board to earn spoons.",
    resultMeta: "Hints used: {hints}",
    round: "Time Attack {current}/{total}",
    elapsed: "Time {time}",
    remaining: "Time left {time}"
  },
  guide: {
    practiceLabel: "A practice row with Pip",
    practicePrompt: "Slide from the first cell to the last one.",
    practiceCell: "Cell {number}",
    practiceTryAgain: "Keep your finger down and color all five cells.",
    practiceComplete: "That’s it. A 5 means five connected cells.",
    practiceSeparated: "Choose three separate cells.",
    practiceSeparatedTryAgain: "Try three cells with a gap between them.",
    practiceSeparatedComplete: "Exactly. Each 1 is its own group.",
    eyebrow: "Pip's cozy guide",
    speaker: "Pip is talking with you",
    skip: "Not now",
    next: "Next",
    done: "Ready? Go!",
    allPuzzlesDone: "This stage is complete!",
    unlockNextHint: "Complete its Pantry shelf to open the next stage. Let's go to the Pantry now 🏺",
    goToPantry: "Fill the Pantry shelf",
    goToSpoonRun: "Collect Spoons",
    puzzle: {
      speakerName: "Pip",
      title: "I'll solve the first picture beside you",
      step1: "Oh, hello! I'm Pip. Welcome to the Picture Pantry!",
      step2: "A 5 means all five cells are connected. Slide across the row in one stroke.",
      step3: "1 1 1 means three single groups. Leave at least one blank between each group."
    },
    cursorControlsIntro: {
      speakerName: "Pip",
      step1: "Choose Color or Blank first. Each arrow move marks the square you enter.",
      step2: "Press and hold an arrow to paint several squares in a smooth trail. Trail Paint starts on and can be turned off in Settings.",
      step3: "Use the switch beside Color and Blank to change between the D-pad and direct taps anytime. You can still choose a default in Settings."
    },
    pantryJarIntro: {
      speakerName: "Pip",
      step1: "Choose Display on home for an owned collectible to place it beside me in the Puzzle Room and on picture-completion screens.",
      step2: "Completing Pantry shelves raises your chance of getting 1 extra spoon after a picture. Check the current +chance on your home collectible or in the Pantry."
    },
    spoonRunIntro: {
      speakerName: "Pip",
      step1: "Solve Today's Picture for bonus spoons. A new picture waits for you every day!",
      step2: "You can challenge pictures you've already finished, too. Earn spoons from clean replays up to 3 times a day!"
    },
    timeAttack: {
      speakerName: "Grandpa Clock",
      title: "Mr. Park's quick challenge",
      step1: "You have 3 minutes total to solve 5×5, 8×8, then 10×10. The clock keeps running between boards.",
      step2: "Finish all three to set a record. If time runs out, every correct cell you reached still counts toward your result.",
      step3: "Hints are optional. They cost 2, then 4, then 7 spoons and slightly lower your score, so save them for when you truly need one."
    },
    map: {
      speakerName: "Pip",
      step1: "Oh, look! This is your badge shelf — it grows as you solve pictures.",
      step2: "Finish a full set to light up its badge.",
      step3: "Your shelf fills up as the pantry grows."
    },
    pantryFirstPurchase: {
      title: "Your first pantry collectible",
      step1: "Lovely choice. Your first collectible is now in the Pantry.",
      step2: "Tap an owned collectible and choose Display on home to place it beside me in the Puzzle Room.",
      step3: "Complete shelves to raise your chance of getting an extra spoon after a picture."
    },
  },
  stageNav: {
    position: "{current}/{total}",
    previous: "Previous",
    list: "Picture list",
    next: "Next"
  },
  playPause: {
    title: "Puzzle paused",
    continue: "Continue puzzle",
    home: "Pip's Puzzle Room",
    pictures: "Picture list"
  },
  controls: {
    fill: "Color",
    mark: "Blank Check",
    markHint: "Check squares you are sure should stay empty.",
    undo: "Undo last move",
    cursorPanel: "Large board controls",
    cursorHint: "Move with arrows, then choose Color or Blank.",
    cursorPosition: "Row {row}, Column {column}",
    cursorStatusLabel: "Selected square: {status}",
    cursorStatusEmpty: "Empty",
    cursorStatusFilled: "Colored",
    cursorStatusMarked: "Blank",
    lineCompleteHint: "When a line is complete, safe blanks become X marks.",
    cursorUp: "Move up",
    cursorDown: "Move down",
    cursorLeft: "Move left",
    cursorRight: "Move right",
    cursorFill: "Color",
    cursorClearFill: "Clear Color",
    cursorMark: "Blank",
    cursorClearMark: "Clear X",
    hint: "Use hint",
    hintWithCost: "Use hint",
    hintConfirmTitle: "Use spoons for a hint?",
    hintConfirmBody: "Use {cost} spoons?",
    hintCancel: "Not now",
    hintConfirmAction: "Use {cost}",
    hintCostLabel: "{cost} spoons",
    hintRemaining: "Hints {count}/{limit}",
    hintMeterLabel: "{count} of {limit} starter hints left",
    extraHintTitle: "Extra hint",
    timeAttackHintTitle: "Time Attack hint",
    hintIntro: "Use a hint?",
    timeAttackHintIntro: "Use {cost} spoons for a hint? You have {balance}.",
    timeAttackHintNeedMore: "You need {cost} spoons. You have {balance}.",
    paidHintIntro: "Use {cost} spoons for a hint? You have {balance}.",
    paidHintNeedMore: "You need {cost} spoons. You have {balance}.",
    hintEmpty: "No hints left for this picture."

  },
  progress: {
    filled: "{count} filled",
    filledOf: "{count} / {target}",
    revisit: "{count} filled - {mistakes} to revisit",
    revisitOf: "{count} / {target} · {mistakes} to revisit",
    lineGuided: "{count} line",
    lineGuidedAria: "{count} completed clue line",
    linesGuided: "{count} lines",
    linesGuidedAria: "{count} completed clue lines",
    complete: "Card complete"
  },
  completion: {
    saved: "A cozy {imageName}! Saved to the Pantry Album.",
    albumCard: "Album card",
    savedStamp: "Saved",
    rewardFactAlbum: "Album saved",
    rewardFactRoom: "Room path grows",
    rewardFactNext: "Next picture ready",
    viewAlbum: "View Album",
    menu: "Menu",
    nextPicture: "Next Picture",
    dailyComplete: "Today's Picture complete!",
    dailyDone: "Daily picture complete!",
    confirm: "OK",
    puzzleReward: "Puzzle complete +{count}sp",
    dailyBonus: "Daily bonus +{count}sp",
    stageBonus: "Shelf completion bonus +{count}sp",
    jarEffectBonus: "Extra spoons · Pantry growth bonus +{count}sp",
    dailyReward: "Today's Picture complete! You earned +{count} spoons.",
    replayReward: "Clean replay! +{count} spoon. {remaining} replay rewards left today.",
    replayNoReward: "Replay complete. No spoon this time, but the card stayed safe.",
    replayExhausted: "That's all for today's replays! Come back tomorrow 🍀",
    backToSpoonRun: "Back to Collect Spoons"
  },
  stageComplete: {
    ariaLabel: "Shelf complete",
    eyebrow: "A new picture with Pip",
    message: "Pip has tucked these pictures neatly onto the shelf.",
    bonus: "+{count} spoons",
    cta: "Picture list",
    finalAriaLabel: "Pip's pantry complete",
    finalEyebrow: "Pip's pantry",
    finalMessage: "The pantry Pip filled with you is complete.",
    finalCta: "See the pantry"
  },
  album: {
    completed: "{completed} pictures",
    genericSaved: "{title} is saved in the album.",
    hiddenTitle: "Hidden picture",
    hiddenText: "Finish the puzzle to save this card.",
    emptyTitle: "Ready to finish your first picture?",
    emptyAction: "Choose a picture",
    lockedSymbol: "?"
  },
  puzzlePicker: {
    size: "{size}×{size}",
    sizeCount: "{count} {size}×{size} puzzles",
    complete: "Complete",
    stageComplete: "Complete",
    completedShelfSummary: "{count} pictures complete · Open to replay",
    sizeComplete: "{size}x{size} - Complete",
    expandShelf: "Expand {title}",
    collapseShelf: "Collapse {title}"
  },
  shelves: {
    pipsFirst: "Pip's First Shelf",
    sunnyCounter: "Sunny Counter",
    sunnyCounterTeaser: "Spoons and recipes await.",
    apronDrawer: "Apron Drawer",
    apronDrawerTeaser: "Find tools in the apron drawer.",
    marketCounter: "Market Counter",
    marketCounterTeaser: "Fresh finds fill the counter.",
    windowTable: "Window Table",
    windowTableTeaser: "Sunlight warms the table.",
    morningBakery: "Morning Bakery",
    morningBakeryTeaser: "Morning baking fills the air.",
    pastryCorner: "Pastry Corner",
    pastryCornerTeaser: "Sweet pastries fill the corner.",
    tinRow: "Tin Row",
    tinRowTeaser: "Baking tins line the shelf.",
    bakeryWindow: "Bakery Window",
    bakeryWindowTeaser: "See the street beyond the window.",
    villageSquare: "Village Square",
    villageSquareTeaser: "A lively village day awaits.",
    marketTable: "Market Table",
    marketTableTeaser: "Seasonal ingredients fill the table.",
    clockCorner: "Clock Corner",
    clockCornerTeaser: "Time drifts through the corner.",
    bakeryWalk: "Bakery Walk",
    bakeryWalkTeaser: "Discoveries line the bakery walk.",
    gardenPath: "Garden Path",
    gardenPathTeaser: "Follow herbs and flowers.",
    villagePantry: "Village Pantry",
    villagePantryTeaser: "Village treasures gather here. 🌿",
    herbTerrace: "Herb Terrace",
    herbTerraceTeaser: "Herbs glow in the morning light.",
    sunroomTable: "Sunroom Table",
    sunroomTableTeaser: "Village keepsakes await Pip.",
    orchardWindow: "Orchard Window",
    orchardWindowTeaser: "Orchard treasures glow nearby.",
    lanternCourtyard: "Lantern Courtyard",
    lanternCourtyardTeaser: "Follow the evening lamps.",
    moonlitVeranda: "Moonlit Veranda",
    moonlitVerandaTeaser: "Keepsakes rest beneath the moon.",
    hearthGallery: "Hearth Gallery",
    hearthGalleryTeaser: "Treasures gather by the hearth.",
    summerWindow: "Summer Window",
    summerWindowTeaser: "Fruit glows in summer light.",
    fruitMarket: "Fruit Market",
    fruitMarketTeaser: "Ripe fruit fills the market.",
    gardenBasket: "Garden Basket",
    gardenBasketTeaser: "Garden harvest fills the baskets.",
    picnicLawn: "Picnic Lawn",
    picnicLawnTeaser: "Cool treats wait in the shade.",
    seasideTable: "Seaside Table",
    seasideTableTeaser: "Sea breeze meets the table.",
    sunsetFeast: "Sunset Feast",
    sunsetFeastTeaser: "Pip’s evening feast glows.",
    koreanHarvest: {
      "moonrise-table": "Moonrise Table",
      "moonrise-tableTeaser": "Half moons, songpyeon, pears, and chestnuts welcome the harvest moon",
      "harvest-courtyard": "Harvest Courtyard",
      "harvest-courtyardTeaser": "Persimmons, rice sheaves, bojagi, and lanterns gather in the courtyard",
      "moon-jar-room": "Moon Jar Room",
      "moon-jar-roomTeaser": "Korean craft and quiet moonlight fill Pip's special room",
      "full-moon-feast": "Full Moon Feast",
      "full-moon-feastTeaser": "Finish the celebration with Pip beneath the bright full moon"
    },
    lockConditionPuzzle: "Solve {count} more on this shelf",
    lockConditionPuzzleDone: "Previous shelf complete ✓",
    lockConditionPantry: "Own {count} more paid Pantry collectible(s)",
    lockConditionPantryDone: "Pantry requirement met ✓"
  },
  shelf: {
    requiresPantryShelf: "Finish {shelf}",
    pantryProgress: "{current}/{total} owned"
  },
  koreanHarvest: {
    rewards: {
      title: "Harvest Moon Keepsakes",
      progress: "{current}/{total} pictures",
      "songpyeon-tray": "Songpyeon Soban",
      "bojagi-gift": "Bojagi Gift",
      "moon-jar": "Moon Jar",
      "moonlit-lantern": "Moonlit Hanji Lantern",
      unlocked: "Collected",
      unlockAt: "Complete {count} pictures"
    }
  },
  packs: {
    free: "Included",
    unlockable: "Unlockable",
    bonusPack: "Future set",
    locked: "Locked",
    progress: "{completed}/{total}",
    preview: "Preview",
    pricePreview: "Preview set",

    futurePackHint: "Future optional sets are hidden until the pantry decoration economy is ready.",
    comingSoon: "Coming soon",
    needPantryRoom: "Decorate the Pantry first",
    visitPantry: "Decorate Pantry",
    roomRequirement: "Decor {completed}/{required}",
    roomRequirementHint: "Decorate your pantry room to unlock this stage.",
    packComplete: "'{title}' complete! Badge earned.",
    "pips-first-shelf": {
      title: "Pip's First Shelf",
      note: "Start with tiny cozy pictures"
    },
    "sunny-spoon-sign": {
      title: "Sunny Spoon Sign",
      note: "Small shop-sign pictures"
    },
    "apron-drawer": {
      title: "Apron Drawer",
      note: "Aprons, tools, and warm kitchen shapes"
    },
    "bakery-window": {
      title: "Bakery Window",
      note: "Bread, sweets, and window-light pictures"
    },
    "village-pantry": {
      title: "Village Pantry",
      note: "Village pantry badge pictures"
    },
    "summer-pantry": {
      title: "Summer Pantry",
      note: "A sunny season of markets, gardens, picnics, and seaside treats"
    },
    "korean-harvest": {
      title: "Korean Harvest Moon",
      note: "Sixteen warm Chuseok pictures beneath the full moon"
    },
    "cafe-window-plus": {
      title: "Cafe Window Plus",
      note: "Optional theme stage planned for later"
    },
    "bakery-morning-plus": {
      title: "Bakery Morning Plus",
      note: "A future optional set for a bakery morning picture"
    },
    "seasonal-pantry-plus": {
      title: "Seasonal Pantry Plus",
      note: "A future optional set for a seasonal pantry picture"
    },
    "village-picnic-plus": {
      title: "Village Picnic Plus",
      note: "A future optional set for a village picnic picture"
    },
    "sunny-festival-plus": {
      title: "Sunny Festival Plus",
      note: "A future optional set for a Sunny Spoon festival picture"
    }
  },
  pantry: {
    title: "Pip's Pantry",
    shelfUnlocksStage: "Complete this shelf to open {stage}",
    shelfStageUnlocked: "{stage} opened ✓",
    spoonNote: "Saved spoons: {count}",
    roomAria: "Pip's decorated pantry",
    shopEyebrow: "Decoration pantry",
    shopTitle: "Decorations",
    shopBody: "Choose a room spot, then pick a decoration that belongs there. Each spot shows one favorite at a time.",
    placementNote: "{count}/{total} pantry spots are decorated. Tap a spot to shop for that place.",
    story: {
      startTitle: "Can we warm up the {slot}?",
      startAction: "Show this item",
      placeTitle: "{item} is ready to place",
      placeAction: "Show the spot"
    },
    storyNextArrival: "Next decoration",
    storyNextArrivalAction: "View {item}",
    storyDeliveryTitle: "Prepare {item}",
    storyDeliveryNeed: "{slot} · {needed} more spoons",
    storyDeliveryReady: "{slot} · Ready",
    storyDeliveryShowGoal: "Show this goal",
    storyDeliveryEarn: "Collect spoons",
    feedbackStoryCompleteTitle: "{item} finished Pip's note",
    feedbackBuyTitle: "{item} is home",
    feedbackEquipTitle: "{item} is on display",
    feedbackDismiss: "Done",
    allSlots: "All spots",
    allRarities: "All grades",
    slotFilterLabel: "Pantry decoration spots",
    rarityFilterLabel: "Pantry decoration grades",
    availabilityFilterLabel: "Pantry decoration availability",
    availability: {
      all: "All items",
      canBuy: "Can buy",
      owned: "Owned"
    },
    sortLabel: "Pantry decoration sorting",
    sortTitle: "Sort",
    sortOptions: {
      featured: "Recommended",
      priceLow: "Low price",
      priceHigh: "High price",
      rarity: "Rare first"
    },
    emptyFilterTitle: "No decorations match",
    emptyFilterBody: "Try another spot, grade, or availability filter.",
    resetFilters: "Reset filters",
    clearFilters: "Clear filters",
    filterSummary: "Showing {count}/{total} decorations",
    filterSummaryAll: "Showing all {count} decorations",
    shopLimitSummary: "Showing {visible}/{total} items so the shop stays easy to scan.",
    shopLimitAction: "Show more decorations",
    rarities: {
      starter: "Starter",
      common: "Common",
      cozy: "Cozy",
      rare: "Rare",
      premium: "Premium"
    },
    slotAction: "Shop decorations for {slot}",
    shelf: {
      summerOrchard: "Summer Orchard",
      sunnyGarden: "Sunny Garden",
      picnicTable: "Picnic Table"
    },
    collectible: {
      watermelonBasket: "Watermelon Basket",
      peachBasket: "Peach Basket",
      cherryBowl: "Cherry Bowl",
      berryPunnet: "Berry Punnet",
      melonCrate: "Melon Crate",
      plumBasket: "Ribboned Plum Basket",
      tomatoTrug: "Tomato Trug",
      sweetCornBasket: "Sweet Corn Basket",
      cucumberTray: "Cucumber Tray",
      basilBundle: "Basil Bundle",
      lemonBasket: "Lemon Basket",
      pepperBunch: "Sweet Pepper Bunch",
      lemonadePitcher: "Lemonade Pitcher",
      fruitTart: "Fruit Tart",
      picnicSandwiches: "Picnic Sandwiches",
      summerSalad: "Summer Salad",
      berryShortcake: "Berry Shortcake",
      shavedIceBowl: "Shaved Ice Bowl"
    },
    emptySlot: "Waiting for a cozy touch",
    owned: "Owned",
    equipped: "Equipped",
    equip: "Equip",
    equipToSlot: "Place in {slot}",
    buy: "Buy",
    replaceItem: "Replace",
    needMore: "Need {count} spoons",
    addSpoons: "Add spoons",
    free: "Starter",
    spoonCost: "{count} spoons",
    rewardPlanAria: "Pantry reward flow",
    rewardPlanTitle: "Spoons will become decoration supplies",
    rewardPlanPuzzle: "Solve puzzles to earn spoons.",
    rewardPlanStage: "Finish stages for bigger spoon rewards.",
    rewardPlanDecorate: "When new pantry items are ready, spend spoons to decorate.",
    slots: {
      backWall: "Back wall",
      counter: "Counter",
      window: "Window",
      shelf: "Shelf",
      floor: "Floor"
    },
    shelf: {
      jam: "Jam & Fruit Preserves",
      honey: "Honey & Syrups",
      herb: "Herbs & Flowers",
      spice: "Seeds & Spices",
      pickle: "Pickles",
      fruit: "Fruit Preserves",
      oil: "Herb Oils",
      tea: "Botanical Teas",
      sunroom: "Sunroom Botanicals",
      orchard: "Orchard Preserves",
      hearth: "Hearthside Treats",
      summerOrchard: "Summer Orchard",
      sunnyGarden: "Sunny Garden",
      picnicTable: "Picnic Table"
    },
    jar: {
      balance: "🥄 {count}",
      openDetail: "View {item}",
      spoonCost: "{count} 🥄",
      shelfLabel: "{shelf}",
      buyAction: "Buy for {count} 🥄",
      needSpoons: "{count} more spoons needed",
      openSpoonStore: "Get more spoons",
      needSpoonsAction: "{count} more spoons needed. Get more spoons",
      close: "Close",
      featureOnHome: "Display on home",
      featuredOnHome: "Displayed on home",
      growthEffectLabel: "Extra-spoon chance",
      growthEffectChance: "+{chance}%",
      growthEffectChanceLocked: "Locked",
      growthEffectDescription: "Complete a picture for a {chance}% chance to earn {reward} bonus spoon.",
      growthEffectDescriptionLocked: "Complete one shelf to unlock a chance of earning a picture completion bonus.",
      growthEffectProgress: "{completed}/{cap} shelves complete",
      growthBonusSummary: "Extra-spoon chance +{chance}%",
      growthBonusSummaryLocked: "Complete a shelf for extra spoons",
      shelfBonusCelebration: "Extra-spoon chance grew to +{chance}%!",
      growthBadgeCompact: "+{chance}%",
      growthBadgeAria: "Extra-spoon chance: plus {chance} percent",
      effectProgress: "Effect progress {progress} / {target}",
      effectProgressBanked: "Banked progress {progress} / {target} · pays on the next eligible day",
      featuredAria: "{item}, displayed collectible. Open Pantry",
      todaysPantry: "Today's Pantry",
      onboardingTitle: "Your shelves are ready.",
      onboardingPrompt: "Choose your first collectible and begin filling Pip's pantry!",
      rarity: {
        starter: "Starter",
        common: "Common",
        rare: "Rare",
        special: "Special",
        luxury: "Luxury"
      },
      strawberryJam: "Strawberry Jam",
      blueberryJam: "Blueberry Jam",
      cherryJam: "Cherry Jam",
      orangeMarmalade: "Orange Marmalade",
      lemonCurd: "Lemon Curd",
      peachPreserve: "Peach Preserve",
      acaciaHoney: "Acacia Honey",
      mapleSyrup: "Maple Syrup",
      yuzuSyrup: "Yuzu Syrup",
      gingerSyrup: "Ginger Syrup",
      maesil: "Green Plum Syrup",
      lavenderHoney: "Lavender Honey",
      rosemary: "Dried Rosemary",
      chamomile: "Chamomile Flowers",
      driedLavender: "Dried Lavender",
      driedMint: "Dried Mint",
      rosePetals: "Dried Rose Petals",
      hibiscus: "Hibiscus",
      seaSalt: "Coarse Salt",
      blackPepper: "Black Pepper",
      cinnamon: "Cinnamon Powder",
      blackSesame: "Black Sesame",
      pumpkinSeeds: "Pumpkin Seeds",
      redBean: "Red Bean",
      cucumberClassic: "Classic Cucumber Pickles",
      dillPickle: "Dill Pickles",
      breadButterPickle: "Bread & Butter Pickles",
      spicyPickle: "Spicy Pickles",
      cornichonJar: "Cornichons",
      kimchiCrock: "Kimchi Crock",
      strawberrySyrup: "Strawberry Syrup",
      yuzuPreserve: "Yuzu Preserve",
      blueberryCompote: "Blueberry Compote",
      apricotPreserve: "Apricot Preserve",
      roseHipSyrup: "Rose Hip Syrup",
      figHoneyPreserve: "Fig & Honey Preserve",
      plainOliveOil: "Plain Olive Oil",
      rosemaryOil: "Rosemary Oil",
      chiliInfusedOil: "Chili Infused Oil",
      garlicHerbOil: "Garlic Herb Oil",
      lemonThymeOil: "Lemon Thyme Oil",
      truffleOilBottle: "Truffle Oil",
      classicTeaBlend: "Classic Tea Blend",
      chamomileLoose: "Loose Chamomile",
      lavenderBlend: "Lavender Blend",
      rosePetalTea: "Rose Petal Tea",
      jasminePearl: "Jasmine Pearls",
      spiceChaiBlend: "Spiced Chai Blend",
      pressedFernTonic: "Pressed Fern Tonic",
      basilDewInfusion: "Basil Dew Infusion",
      mintBlossomCordial: "Mint Blossom Cordial",
      rosemaryCitrusElixir: "Rosemary Citrus Elixir",
      lavenderSageTonic: "Lavender Sage Tonic",
      goldenHerbEssence: "Golden Herb Essence",
      appleBlossomWater: "Apple Blossom Water",
      pearVanillaPreserve: "Pear & Vanilla Preserve",
      plumHoneyCompote: "Plum & Honey Compote",
      quinceSpiceMarmalade: "Spiced Quince Marmalade",
      moonPeachConserve: "Moon Peach Conserve",
      jeweledOrchardPreserve: "Jeweled Orchard Preserve",
      hearthCocoaMix: "Hearth Cocoa Mix",
      toastedHazelnutSpread: "Toasted Hazelnut Spread",
      chestnutVanillaCream: "Chestnut Vanilla Cream",
      emberBerryConserve: "Ember Berry Conserve",
      spicedFigPudding: "Spiced Fig Pudding",
      midnightCaramelReserve: "Midnight Caramel Reserve"
    },
    items: {
      starterCounter: { title: "Starter Counter Cloth", description: "A soft check cloth for Pip's first little serving spot." },
      sunnyCurtains: { title: "Sunny Window Curtains", description: "Warm curtains that make the pantry feel freshly opened." },
      recipeShelf: { title: "Recipe Card Shelf", description: "Tiny cards and jars for Pip's favorite pantry notes." },
      mintRug: { title: "Mint Check Rug", description: "A fresh floor rug that keeps the room gentle and bright." },
      smallJamJar: { title: "Small Jam Jar", description: "A sweet little jar for the counter's sunny corner." },
      herbPot: { title: "Herb Pot", description: "A bright windowsill pot that keeps the pantry feeling fresh." },
      corkBoard: { title: "Recipe Cork Board", description: "Pinned notes and tiny cards for the back wall." },
      tinySucculent: { title: "Tiny Succulent", description: "A soft green shelf friend in a little measuring cup." },
      spoonWallClock: { title: "Spoon Wall Clock", description: "A honey-rimmed clock that keeps the back wall cheerful." },
      berryTeaTins: { title: "Berry Tea Tins", description: "A neat stack of sweet tins for the pantry shelf." },
      ribbonRollingPin: { title: "Ribbon Rolling Pin", description: "A bakery counter favorite tied with a tiny spoon charm." },
      flowerWindowVase: { title: "Sunny Flower Vase", description: "A small windowsill vase that brightens the room." },
      wovenPantryBasket: { title: "Woven Pantry Basket", description: "A low basket of napkins and treats for the pantry floor." },
      honeyCakeStand: { title: "Honey Cake Stand", description: "A glossy little cake stand for the coziest counter display." },
      laceWindowLantern: { title: "Lace Window Lantern", description: "A warm lantern that makes the windowsill glow softly." },
      copperCookieTin: { title: "Copper Cookie Tin", description: "A polished tin of tiny cookies for the shelf." },
      plushFloorCushion: { title: "Plush Floor Cushion", description: "A soft round cushion that makes the pantry floor inviting." },
      framedRecipeGlow: { title: "Framed Recipe Glow", description: "A softly glowing recipe frame for the back wall." },
      goldenWafflePress: { title: "Golden Waffle Press", description: "A rare golden press with a tiny heart waffle tucked inside." },
      stainedGlassSuncatcher: { title: "Stained Glass Suncatcher", description: "A rare sunlit spoon charm for the brightest window." },
      porcelainSpiceCarousel: { title: "Porcelain Spice Carousel", description: "A rare rotating spice set with polished little jars." },
      pantryDeliveryCart: { title: "Pantry Delivery Cart", description: "A rare rolling cart stacked with cozy pantry treats." },
      spoonWallTapestry: { title: "Spoon Wall Tapestry", description: "A rare stitched wall hanging for Sunny Spoon pride." },
      soupPot: { title: "Soup Pot Display", description: "A cozy pot display for the counter's happiest corner." },
      goldenSign: { title: "Golden Spoon Sign", description: "A shiny Sunny Spoon sign for big pantry pride." }
    }
  },
  map: {
    count: "{completed}/{total} cards complete",
    note: "Each completed stage becomes a badge.",
    lockedWithPantry: "Locked - Pantry room step {completed}/{required}",
    inProgress: "In progress",
    revealed: "Revealed",
    locked: "Locked",
    nextSets: "Future picture sets",
    sets: {
      "pip-portrait": "Pip portrait",
      "cozy-cafe-room": "Cozy cafe room",
      "bakery-morning": "Bakery morning",
      "seasonal-pantry": "Seasonal pantry",
      "village-picnic": "Village picnic",
      "sunny-spoon-festival": "Sunny Spoon festival"
    },
    parts: {
      "pip-hat": "Chef hat",
      "pip-scarf": "Red scarf",
      "pip-face": "Pip face",
      "pip-body": "Pip body",
      "pip-card": "Picture card",
      "bonus-cafe": "Bonus scene",
      "bonus-bakery": "Bakery scene",
      "bonus-season": "Seasonal scene",
      "bonus-picnic": "Picnic scene",
      "bonus-festival": "Festival scene"
    }
  },
  puzzles: {
    ...summerPuzzleCopy,
    "korean-harvest-half-moon-1": { title: "Half Moon", imageName: "Half Moon" },
    "korean-harvest-songpyeon-2": { title: "Songpyeon", imageName: "Songpyeon" },
    "korean-harvest-korean-pear-3": { title: "Korean Pear", imageName: "Korean Pear" },
    "korean-harvest-chestnut-4": { title: "Chestnut", imageName: "Chestnut" },
    "korean-harvest-persimmon-branch-5": { title: "Persimmon Branch", imageName: "Persimmon Branch" },
    "korean-harvest-rice-sheaf-6": { title: "Rice Sheaf", imageName: "Rice Sheaf" },
    "korean-harvest-bojagi-knot-7": { title: "Bojagi Knot", imageName: "Bojagi Knot" },
    "korean-harvest-paper-lantern-8": { title: "Paper Lantern", imageName: "Paper Lantern" },
    "korean-harvest-moon-jar-9": { title: "Moon Jar", imageName: "Moon Jar" },
    "korean-harvest-celadon-bottle-10": { title: "Celadon Bottle", imageName: "Celadon Bottle" },
    "korean-harvest-tiled-roof-moon-11": { title: "Tiled Roof Moon", imageName: "Tiled Roof Moon" },
    "korean-harvest-lacquer-tray-12": { title: "Lacquer Tray", imageName: "Lacquer Tray" },
    "korean-harvest-full-moon-roof-13": { title: "Full Moon Roof", imageName: "Full Moon Roof" },
    "korean-harvest-harvest-basket-14": { title: "Harvest Basket", imageName: "Harvest Basket" },
    "korean-harvest-rabbit-rice-cake-15": { title: "Rabbit Rice Cake", imageName: "Rabbit Rice Cake" },
    "korean-harvest-pip-in-hanbok-16": { title: "Pip in Hanbok", imageName: "Pip in Hanbok" },
    "stability-tea-tray-113": { title: "Tea Tray", imageName: "Tea Tray" },
    "stability-breakfast-tray-114": { title: "Breakfast Tray", imageName: "Breakfast Tray" },
    "stability-kitchen-timer-115": { title: "Kitchen Timer", imageName: "Kitchen Timer" },
    "stability-egg-timer-116": { title: "Egg Timer", imageName: "Egg Timer" },
    "stability-potted-fern-117": { title: "Potted Fern", imageName: "Potted Fern" },
    "stability-window-fern-118": { title: "Window Fern", imageName: "Window Fern" },
    "stability-chef-hat-119": { title: "Chef Hat", imageName: "Chef Hat" },
    "stability-baker-hat-120": { title: "Baker Hat", imageName: "Baker Hat" },
    "stability-letter-basket-121": { title: "Letter Basket", imageName: "Letter Basket" },
    "stability-recipe-basket-122": { title: "Recipe Basket", imageName: "Recipe Basket" },
    "stability-handbell-123": { title: "Handbell", imageName: "Handbell" },
    "stability-dinner-bell-124": { title: "Dinner Bell", imageName: "Dinner Bell" },
    "stability-patchwork-quilt-125": { title: "Patchwork Quilt", imageName: "Patchwork Quilt" },
    "stability-picnic-blanket-126": { title: "Picnic Blanket", imageName: "Picnic Blanket" },
    "stability-little-pantry-key-127": { title: "Little Pantry Key", imageName: "Little Pantry Key" },
    "stability-window-planter-128": { title: "Window Planter", imageName: "Window Planter" },
    "stability-herb-planter-129": { title: "Herb Planter", imageName: "Herb Planter" },
    "stability-copper-pan-130": { title: "Copper Pan", imageName: "Copper Pan" },
    "stability-skillet-pan-131": { title: "Skillet Pan", imageName: "Skillet Pan" },
    "stability-market-bicycle-132": { title: "Market Bicycle", imageName: "Market Bicycle" },
    "stability-bakery-bicycle-133": { title: "Bakery Bicycle", imageName: "Bakery Bicycle" },
    "stability-cake-dome-134": { title: "Cake Dome", imageName: "Cake Dome" },
    "stability-pastry-dome-135": { title: "Pastry Dome", imageName: "Pastry Dome" },
    "stability-sunflower-wreath-136": { title: "Sunflower Wreath", imageName: "Sunflower Wreath" },
    "stability-daisy-wreath-137": { title: "Daisy Wreath", imageName: "Daisy Wreath" },
    "stability-recipe-board-138": { title: "Recipe Board", imageName: "Recipe Board" },
    "stability-menu-board-139": { title: "Menu Board", imageName: "Menu Board" },
    "stability-linen-basket-140": { title: "Linen Basket", imageName: "Linen Basket" },
    "stability-napkin-basket-141": { title: "Napkin Basket", imageName: "Napkin Basket" },
    "stability-porch-rocker-142": { title: "Porch Rocker", imageName: "Porch Rocker" },
    "stability-reading-rocker-143": { title: "Reading Rocker", imageName: "Reading Rocker" },
    "stability-berry-colander-144": { title: "Berry Colander", imageName: "Berry Colander" },
    "stability-apple-colander-145": { title: "Apple Colander", imageName: "Apple Colander" },
    "stability-garden-trellis-146": { title: "Garden Trellis", imageName: "Garden Trellis" },
    "stability-rose-trellis-147": { title: "Rose Trellis", imageName: "Rose Trellis" },
    "stability-pantry-stool-148": { title: "Pantry Stool", imageName: "Pantry Stool" },
    "stability-kitchen-stool-149": { title: "Kitchen Stool", imageName: "Kitchen Stool" },
    "stability-bread-cloche-150": { title: "Bread Cloche", imageName: "Bread Cloche" },
    "stability-bun-cloche-151": { title: "Bun Cloche", imageName: "Bun Cloche" },
    "stability-porch-swing-152": { title: "Porch Swing", imageName: "Porch Swing" },
    "stability-garden-swing-153": { title: "Garden Swing", imageName: "Garden Swing" },
    "stability-spice-rack-154": { title: "Pantry Spice Rack", imageName: "Pantry Spice Rack" },
    "stability-herb-rack-155": { title: "Herb Rack", imageName: "Herb Rack" },
    "stability-orchard-ladder-156": { title: "Orchard Ladder", imageName: "Orchard Ladder" },
    "stability-pantry-ladder-157": { title: "Pantry Ladder", imageName: "Pantry Ladder" },
    "stability-tea-trolley-158": { title: "Tea Trolley", imageName: "Tea Trolley" },
    "stability-dessert-trolley-159": { title: "Dessert Trolley", imageName: "Dessert Trolley" },
    "stability-cottage-window-160": { title: "Cottage Window", imageName: "Cottage Window" },
    "stability-pantry-window-161": { title: "Pantry Window", imageName: "Pantry Window" },
    "stability-jam-kettle-162": { title: "Jam Kettle", imageName: "Jam Kettle" },
    "stability-preserve-kettle-163": { title: "Preserve Kettle", imageName: "Preserve Kettle" },
    "stability-village-lamppost-164": { title: "Village Lamppost", imageName: "Village Lamppost" },
    "stability-garden-lamppost-165": { title: "Garden Lamppost", imageName: "Garden Lamppost" },
    "stability-pantry-cabinet-166": { title: "Pantry Cabinet", imageName: "Pantry Cabinet" },
    "stability-recipe-cabinet-167": { title: "Recipe Cabinet", imageName: "Recipe Cabinet" },
    "stability-counter-bell-57": { title: "Counter Bell", imageName: "Counter Bell" },
    "stability-porch-bell-58": { title: "Porch Bell", imageName: "Porch Bell" },
    "stability-cocoa-mug-59": { title: "Cocoa Mug", imageName: "Cocoa Mug" },
    "stability-cinnamon-mug-60": { title: "Cinnamon Mug", imageName: "Cinnamon Mug" },
    "stability-honey-dipper-61": { title: "Honey Dipper", imageName: "Honey Dipper" },
    "stability-jam-spoon-62": { title: "Jam Spoon", imageName: "Jam Spoon" },
    "stability-rosemary-sprig-63": { title: "Rosemary Sprig", imageName: "Rosemary Sprig" },
    "stability-mint-sprig-64": { title: "Mint Sprig", imageName: "Mint Sprig" },
    "stability-oven-mitt-65": { title: "Oven Mitt", imageName: "Oven Mitt" },
    "stability-quilted-mitt-66": { title: "Quilted Mitt", imageName: "Quilted Mitt" },
    "stability-picnic-lamp-67": { title: "Picnic Lamp", imageName: "Picnic Lamp" },
    "stability-garden-lamp-68": { title: "Garden Lamp", imageName: "Garden Lamp" },
    "stability-ribbon-bow-69": { title: "Ribbon Bow", imageName: "Ribbon Bow" },
    "stability-gift-bow-70": { title: "Gift Bow", imageName: "Gift Bow" },
    "stability-oak-acorn-71": { title: "Oak Acorn", imageName: "Oak Acorn" },
    "stability-forest-chestnut-72": { title: "Forest Chestnut", imageName: "Forest Chestnut" },
    "stability-sunroom-gazebo-73": { title: "Sunroom Gazebo", imageName: "Sunroom Gazebo" },
    "stability-garden-gazebo-74": { title: "Garden Gazebo", imageName: "Garden Gazebo" },
    "stability-flower-wheelbarrow-75": { title: "Flower Wheelbarrow", imageName: "Flower Wheelbarrow" },
    "stability-herb-wheelbarrow-76": { title: "Herb Wheelbarrow", imageName: "Herb Wheelbarrow" },
    "stability-honey-beehive-77": { title: "Honey Beehive", imageName: "Honey Beehive" },
    "stability-orchard-beehive-78": { title: "Orchard Beehive", imageName: "Orchard Beehive" },
    "stability-recipe-mailbox-79": { title: "Recipe Mailbox", imageName: "Recipe Mailbox" },
    "stability-village-mailbox-80": { title: "Village Mailbox", imageName: "Village Mailbox" },
    "stability-orchard-bridge-81": { title: "Orchard Bridge", imageName: "Orchard Bridge" },
    "stability-garden-bridge-82": { title: "Garden Bridge", imageName: "Garden Bridge" },
    "stability-rain-umbrella-83": { title: "Rain Umbrella", imageName: "Rain Umbrella" },
    "stability-picnic-parasol-84": { title: "Picnic Parasol", imageName: "Picnic Parasol" },
    "stability-flour-sack-85": { title: "Ribboned Flour Sack", imageName: "Ribboned Flour Sack" },
    "stability-grain-sack-86": { title: "Grain Sack", imageName: "Grain Sack" },
    "stability-pastry-roller-87": { title: "Pastry Roller", imageName: "Pastry Roller" },
    "stability-cinnamon-roller-88": { title: "Cinnamon Roller", imageName: "Cinnamon Roller" },
    "stability-kitchen-scale-89": { title: "Kitchen Scale", imageName: "Kitchen Scale" },
    "stability-market-scale-90": { title: "Market Scale", imageName: "Market Scale" },
    "stability-robin-birdbath-91": { title: "Robin Birdbath", imageName: "Robin Birdbath" },
    "stability-garden-fountain-92": { title: "Garden Fountain", imageName: "Garden Fountain" },
    "stability-tulip-bouquet-93": { title: "Tulip Bouquet", imageName: "Tulip Bouquet" },
    "stability-wildflower-bouquet-94": { title: "Wildflower Bouquet", imageName: "Wildflower Bouquet" },
    "stability-jam-display-shelf-95": { title: "Jam Display Shelf", imageName: "Jam Display Shelf" },
    "stability-tea-display-shelf-96": { title: "Tea Display Shelf", imageName: "Tea Display Shelf" },
    "stability-apron-hook-97": { title: "Apron Hook", imageName: "Apron Hook" },
    "stability-utensil-rack-98": { title: "Utensil Rack", imageName: "Utensil Rack" },
    "stability-cookie-tin-99": { title: "Cookie Tin", imageName: "Cookie Tin" },
    "stability-biscuit-tin-100": { title: "Biscuit Tin", imageName: "Biscuit Tin" },
    "stability-hearth-stove-101": { title: "Hearth Stove", imageName: "Hearth Stove" },
    "stability-cottage-stove-102": { title: "Cottage Stove", imageName: "Cottage Stove" },
    "stability-mixing-bowl-103": { title: "Mixing Bowl", imageName: "Mixing Bowl" },
    "stability-batter-bowl-104": { title: "Batter Bowl", imageName: "Batter Bowl" },
    "stability-berry-crate-105": { title: "Berry Crate", imageName: "Berry Crate" },
    "stability-apple-crate-106": { title: "Apple Crate", imageName: "Apple Crate" },
    "stability-cafe-sign-107": { title: "Cafe Sign", imageName: "Cafe Sign" },
    "stability-bakery-sign-108": { title: "Bakery Sign", imageName: "Bakery Sign" },
    "stability-picnic-table-109": { title: "Picnic Table", imageName: "Picnic Table" },
    "stability-garden-table-110": { title: "Garden Table", imageName: "Garden Table" },
    "stability-pantry-door-111": { title: "Pantry Door", imageName: "Pantry Door" },
    "stability-cottage-door-112": { title: "Cottage Door", imageName: "Cottage Door" },
    "stability-warm-teapot-1": { title: "Warm Teapot", imageName: "Warm Teapot" },
    "stability-window-teapot-2": { title: "Window Teapot", imageName: "Window Teapot" },
    "stability-herb-watering-can-3": { title: "Herb Watering Can", imageName: "Herb Watering Can" },
    "stability-garden-watering-can-4": { title: "Garden Watering Can", imageName: "Garden Watering Can" },
    "stability-forest-mushroom-5": { title: "Forest Mushroom", imageName: "Forest Mushroom" },
    "stability-meadow-mushroom-6": { title: "Meadow Mushroom", imageName: "Meadow Mushroom" },
    "stability-pantry-lantern-7": { title: "Pantry Lantern", imageName: "Pantry Lantern" },
    "stability-porch-lantern-8": { title: "Porch Lantern", imageName: "Porch Lantern" },
    "stability-sparrow-house-9": { title: "Sparrow House", imageName: "Sparrow House" },
    "stability-robin-house-10": { title: "Robin House", imageName: "Robin House" },
    "stability-market-basket-11": { title: "Market Basket", imageName: "Market Basket" },
    "stability-berry-basket-12": { title: "Berry Basket", imageName: "Berry Basket" },
    "stability-daisy-pot-13": { title: "Daisy Pot", imageName: "Daisy Pot" },
    "stability-sunflower-pot-14": { title: "Sunflower Pot", imageName: "Sunflower Pot" },
    "stability-rain-boots-15": { title: "Rain Boots", imageName: "Rain Boots" },
    "stability-garden-boots-16": { title: "Garden Boots", imageName: "Garden Boots" },
    "stability-cafe-chair-17": { title: "Cafe Chair", imageName: "Cafe Chair" },
    "stability-window-chair-18": { title: "Window Chair", imageName: "Window Chair" },
    "stability-bread-basket-19": { title: "Bread Basket", imageName: "Bread Basket" },
    "stability-picnic-basket-20": { title: "Picnic Basket", imageName: "Picnic Basket" },
    "stability-wall-clock-21": { title: "Wall Clock", imageName: "Wall Clock" },
    "stability-kitchen-clock-22": { title: "Kitchen Clock", imageName: "Kitchen Clock" },
    "stability-basil-pot-23": { title: "Basil Pot", imageName: "Basil Pot" },
    "stability-mint-pot-24": { title: "Mint Pot", imageName: "Mint Pot" },
    "stability-jam-cart-25": { title: "Jam Cart", imageName: "Jam Cart" },
    "stability-market-cart-26": { title: "Market Cart", imageName: "Market Cart" },
    "stability-butter-croissant-27": { title: "Golden Butter Croissant", imageName: "Golden Butter Croissant" },
    "stability-honey-croissant-28": { title: "Honey Croissant", imageName: "Honey Croissant" },
    "stability-flour-windmill-29": { title: "Flour Windmill", imageName: "Flour Windmill" },
    "stability-village-windmill-30": { title: "Village Windmill", imageName: "Village Windmill" },
    "stability-copper-kettle-31": { title: "Hearthside Copper Kettle", imageName: "Hearthside Copper Kettle" },
    "stability-tea-kettle-32": { title: "Tea Kettle", imageName: "Tea Kettle" },
    "stability-village-gate-33": { title: "Village Gate", imageName: "Village Gate" },
    "stability-garden-gate-34": { title: "Garden Gate", imageName: "Garden Gate" },
    "stability-striped-apron-35": { title: "Striped Apron", imageName: "Striped Apron" },
    "stability-bakers-apron-36": { title: "Baker's Apron", imageName: "Baker's Apron" },
    "stability-berry-tart-37": { title: "Morning Berry Tart", imageName: "Morning Berry Tart" },
    "stability-cherry-tart-38": { title: "Cherry Tart", imageName: "Cherry Tart" },
    "stability-recipe-book-39": { title: "Recipe Book", imageName: "Recipe Book" },
    "stability-pantry-journal-40": { title: "Pantry Journal", imageName: "Pantry Journal" },
    "stability-window-candle-41": { title: "Window Candle", imageName: "Window Candle" },
    "stability-evening-candle-42": { title: "Evening Candle", imageName: "Evening Candle" },
    "stability-garden-bench-43": { title: "Garden Bench", imageName: "Garden Bench" },
    "stability-village-bench-44": { title: "Village Bench", imageName: "Village Bench" },
    "stability-cake-stand-45": { title: "Cake Stand", imageName: "Cake Stand" },
    "stability-pastry-stand-46": { title: "Pastry Stand", imageName: "Pastry Stand" },
    "stability-milk-churn-47": { title: "Creamery Milk Churn", imageName: "Creamery Milk Churn" },
    "stability-cream-churn-48": { title: "Cream Churn", imageName: "Cream Churn" },
    "stability-hand-mixer-49": { title: "Hand Mixer", imageName: "Hand Mixer" },
    "stability-cream-whisk-50": { title: "Cream Whisk", imageName: "Cream Whisk" },
    "stability-market-awning-51": { title: "Market Awning", imageName: "Market Awning" },
    "stability-bakery-awning-52": { title: "Bakery Awning", imageName: "Bakery Awning" },
    "stability-soup-pot-53": { title: "Soup Pot", imageName: "Soup Pot" },
    "stability-stew-pot-54": { title: "Stew Pot", imageName: "Stew Pot" },
    "stability-cozy-cottage-55": { title: "Cozy Cottage", imageName: "Cozy Cottage" },
    "stability-pantry-cottage-56": { title: "Sunny Pantry Cottage", imageName: "Sunny Pantry Cottage" },
    "pips-first-shelf-pip-face-1": {
      title: "Pip Face",
      imageName: "Pip Face",
      albumText: "Pip saved his first tiny picture."
    },
    "pips-first-shelf-spoon-2-13": { title: "Wooden Spoon", imageName: "Wooden Spoon" },
    "village-pantry-flour-sack-27": {
      title: "Flour Sack",
      imageName: "Flour Sack"
    },
    "village-pantry-spice-rack-28": {
      title: "Spice Rack",
      imageName: "Spice Rack"
    },
    "village-pantry-hanging-herbs-29": {
      title: "Hanging Herbs",
      imageName: "Hanging Herbs"
    },
    "village-pantry-checkered-napkin-30": {
      title: "Checkered Napkin",
      imageName: "Checkered Napkin"
    },
    "village-pantry-candle-shelf-31": {
      title: "Candle Shelf",
      imageName: "Candle Shelf"
    },
    "village-pantry-wicker-tray-32": {
      title: "Wicker Tray",
      imageName: "Wicker Tray"
    },
    "bakery-window-chef-hat-17": {
      title: "Chef Hat",
      imageName: "Chef Hat"
    },
    "bakery-window-cupcake-18": {
      title: "Cupcake",
      imageName: "Cupcake"
    },
    "bakery-window-kettle-19": {
      title: "Kettle",
      imageName: "Kettle"
    },
    "bakery-window-rolling-pin-20": {
      title: "Rolling Pin",
      imageName: "Rolling Pin"
    },
    "bakery-window-glow-21": {
      title: "Bakery Window Glow",
      imageName: "Bakery Window Glow"
    },
    "village-pantry-jam-jar-15": {
      title: "Jam Jar",
      imageName: "Jam Jar"
    },
    "village-pantry-whisk-16": {
      title: "Whisk",
      imageName: "Whisk"
    },
    "village-pantry-chef-hat-17": {
      title: "Chef Hat",
      imageName: "Chef Hat"
    },
    "village-pantry-cupcake-18": {
      title: "Cupcake",
      imageName: "Cupcake"
    },
    "village-pantry-kettle-19": {
      title: "Kettle",
      imageName: "Kettle"
    },
    "village-pantry-rolling-pin-20": {
      title: "Rolling Pin",
      imageName: "Rolling Pin"
    },
    "village-pantry-market-basket-21": {
      title: "Market Basket",
      imageName: "Market Basket"
    },
    "village-pantry-garden-window-22": {
      title: "Garden Window",
      imageName: "Garden Window"
    },
    "village-pantry-picnic-cloth-23": {
      title: "Picnic Cloth",
      imageName: "Picnic Cloth"
    },
    "village-pantry-flower-cart-24": {
      title: "Flower Cart",
      imageName: "Flower Cart"
    },
    "village-pantry-tea-tray-25": {
      title: "Tea Tray",
      imageName: "Tea Tray"
    },
    "village-pantry-jam-crate-26": {
      title: "Jam Crate",
      imageName: "Jam Crate"
    },
    "bakery-window-croissant-22": {
      title: "Croissant Tray",
      imageName: "Croissant Tray"
    },
    "bakery-window-tiered-cakes-23": {
      title: "Tiered Cakes",
      imageName: "Tiered Cakes"
    },
    "bakery-window-macaron-box-24": {
      title: "Macaron Box",
      imageName: "Macaron Box"
    },
    "bakery-window-cocoa-tin-25": {
      title: "Cocoa Tin",
      imageName: "Cocoa Tin"
    },
    "bakery-window-honey-jar-shelf-26": {
      title: "Honey Jar Shelf",
      imageName: "Honey Jar Shelf"
    },
    "bakery-window-berry-tart-27": {
      title: "Berry Tart",
      imageName: "Berry Tart"
    },
    "bakery-window-pie-lattice-28": {
      title: "Pie Lattice",
      imageName: "Pie Lattice"
    },
    "bakery-window-cookie-jar-row-29": {
      title: "Cookie Jar Row",
      imageName: "Cookie Jar Row"
    },
    "bakery-window-scone-basket-30": {
      title: "Scone Basket",
      imageName: "Scone Basket"
    },
    "bakery-window-milk-glass-31": {
      title: "Milk Glass",
      imageName: "Milk Glass"
    },
    "bakery-window-cinnamon-rolls-32": {
      title: "Cinnamon Rolls",
      imageName: "Cinnamon Rolls"
    },
    "bakery-window-cup-stack-33": {
      title: "Cup Stack",
      imageName: "Cup Stack"
    },
    "bakery-window-lemon-tart-34": {
      title: "Lemon Tart",
      imageName: "Lemon Tart"
    },
    "bakery-window-sugar-duster-35": {
      title: "Sugar Duster",
      imageName: "Sugar Duster"
    },
    "bakery-window-pretzel-twist-36": {
      title: "Pretzel Twist",
      imageName: "Pretzel Twist"
    },
    "bakery-window-berry-jam-pot-37": {
      title: "Berry Jam Pot",
      imageName: "Berry Jam Pot"
    },
    "bakery-window-icing-piping-bag-38": {
      title: "Icing Piping Bag",
      imageName: "Icing Piping Bag"
    },
    "bakery-window-cherry-danish-tray-39": {
      title: "Cherry Danish Tray",
      imageName: "Cherry Danish Tray"
    },
    "bakery-window-flower-box-window-40": {
      title: "Flower Box Window",
      imageName: "Flower Box Window"
    },
    "bakery-window-honey-spoon-jar-41": {
      title: "Honey Spoon Jar",
      imageName: "Honey Spoon Jar"
    },
    "bakery-window-cocoa-mug-steam-42": {
      title: "Cocoa Mug Steam",
      imageName: "Cocoa Mug Steam"
    },
    "bakery-window-gingerbread-heart-43": {
      title: "Gingerbread Heart",
      imageName: "Gingerbread Heart"
    },
    "bakery-window-layer-cake-slice-44": {
      title: "Layer Cake Slice",
      imageName: "Layer Cake Slice"
    },
    "bakery-window-ribbon-cookie-box-45": {
      title: "Ribbon Cookie Box",
      imageName: "Ribbon Cookie Box"
    },
    "bakery-window-peach-tart-fan-46": {
      title: "Peach Tart Fan",
      imageName: "Peach Tart Fan"
    },
    "bakery-window-sugar-bell-47": {
      title: "Sugar Bell",
      imageName: "Sugar Bell"
    },
    "bakery-window-jam-thumbprint-48": {
      title: "Jam Thumbprint",
      imageName: "Jam Thumbprint"
    },
    "bakery-window-lemon-glaze-bun-49": {
      title: "Lemon Glaze Bun",
      imageName: "Lemon Glaze Bun"
    },
    "bakery-window-caramel-custard-cup-50": {
      title: "Caramel Custard Cup",
      imageName: "Caramel Custard Cup"
    },
    "bakery-window-berry-cream-roll-51": {
      title: "Berry Cream Roll",
      imageName: "Berry Cream Roll"
    },
    "bakery-window-cocoa-cream-puff-52": {
      title: "Cocoa Cream Puff",
      imageName: "Cocoa Cream Puff"
    },
    "bakery-window-sprinkle-donut-53": {
      title: "Sprinkle Donut",
      imageName: "Sprinkle Donut"
    },
    "bakery-window-cinnamon-swirl-roll-54": {
      title: "Cinnamon Swirl Roll",
      imageName: "Cinnamon Swirl Roll"
    },
    "bakery-window-strawberry-tart-55": {
      title: "Strawberry Tart",
      imageName: "Strawberry Tart"
    },
    "bakery-window-honey-cruller-twist-56": {
      title: "Honey Cruller Twist",
      imageName: "Honey Cruller Twist"
    },
    "bakery-window-pear-galette-57": {
      title: "Pear Galette",
      imageName: "Pear Galette"
    },
    "village-pantry-pickle-crocks-33": {
      title: "Pickle Crocks",
      imageName: "Pickle Crocks"
    },
    "village-pantry-bread-board-34": {
      title: "Bread Board",
      imageName: "Bread Board"
    },
    "village-pantry-copper-ladle-35": {
      title: "Copper Ladle",
      imageName: "Copper Ladle"
    },
    "village-pantry-potato-sack-36": {
      title: "Potato Sack",
      imageName: "Potato Sack"
    },
    "village-pantry-tea-tin-stack-37": {
      title: "Tea Tin Stack",
      imageName: "Tea Tin Stack"
    },
    "village-pantry-market-basket-38": {
      title: "Market Basket",
      imageName: "Market Basket"
    },
    "village-pantry-herb-bundle-39": {
      title: "Herb Bundle",
      imageName: "Herb Bundle"
    },
    "village-pantry-patchwork-tea-cozy-40": {
      title: "Patchwork Tea Cozy",
      imageName: "Patchwork Tea Cozy"
    },
    "village-pantry-blueberry-label-41": {
      title: "Blueberry Label",
      imageName: "Blueberry Label"
    },
    "village-pantry-potted-basil-42": {
      title: "Potted Basil",
      imageName: "Potted Basil"
    },
    "village-pantry-warm-pie-window-43": {
      title: "Warm Pie Window",
      imageName: "Warm Pie Window"
    },
    "village-pantry-checkered-jam-cloth-44": {
      title: "Checkered Jam Cloth",
      imageName: "Checkered Jam Cloth"
    },
    "village-pantry-cinnamon-braid-45": {
      title: "Cinnamon Braid",
      imageName: "Cinnamon Braid"
    },
    "village-pantry-teapot-cozy-46": {
      title: "Teapot Cozy",
      imageName: "Teapot Cozy"
    },
    "village-pantry-copper-kettle-47": {
      title: "Copper Kettle",
      imageName: "Copper Kettle"
    },
    "village-pantry-berry-bowl-48": {
      title: "Berry Bowl",
      imageName: "Berry Bowl"
    },
    "village-pantry-flower-milk-jug-49": {
      title: "Flower Milk Jug",
      imageName: "Flower Milk Jug"
    },
    "village-pantry-toast-rack-50": {
      title: "Toast Rack",
      imageName: "Toast Rack"
    },
    "village-pantry-honey-dipper-51": {
      title: "Honey Dipper",
      imageName: "Honey Dipper"
    },
    "village-pantry-egg-basket-52": {
      title: "Egg Basket",
      imageName: "Egg Basket"
    },
    "village-pantry-cotton-napkin-ring-53": {
      title: "Cotton Napkin Ring",
      imageName: "Cotton Napkin Ring"
    },
    "village-pantry-spice-scoop-54": {
      title: "Spice Scoop",
      imageName: "Spice Scoop"
    },
    "village-pantry-ribbon-tea-tin-55": {
      title: "Ribbon Tea Tin",
      imageName: "Ribbon Tea Tin"
    },
    "village-pantry-checked-pot-holder-56": {
      title: "Checked Pot Holder",
      imageName: "Checked Pot Holder"
    },
    "village-pantry-lace-jar-cover-57": {
      title: "Lace Jar Cover",
      imageName: "Lace Jar Cover"
    },
    "village-pantry-garden-herb-bundle-58": {
      title: "Garden Herb Bundle",
      imageName: "Garden Herb Bundle"
    },
    "bakery-window-apricot-danish-58": {
      title: "Apricot Danish",
      imageName: "Apricot Danish"
    },
    "bakery-window-vanilla-eclair-59": {
      title: "Vanilla Eclair",
      imageName: "Vanilla Eclair"
    },
    "village-pantry-hanging-ladle-59": {
      title: "Hanging Ladle",
      imageName: "Hanging Ladle"
    },
    "village-pantry-pickle-jar-60": {
      title: "Pickle Jar",
      imageName: "Pickle Jar"
    },
    "bakery-window-jam-crescent-60": {
      title: "Jam Crescent",
      imageName: "Jam Crescent"
    },
    "bakery-window-lemon-tartlet-61": {
      title: "Lemon Tartlet",
      imageName: "Lemon Tartlet"
    },
    "village-pantry-flour-sifter-61": {
      title: "Flour Sifter",
      imageName: "Flour Sifter"
    },
    "village-pantry-cocoa-tin-62": {
      title: "Cocoa Scoop Tin",
      imageName: "Cocoa Scoop Tin"
    },
    "bakery-window-braided-pretzel-62": {
      title: "Braided Pretzel",
      imageName: "Braided Pretzel"
    },
    "bakery-window-berry-danish-square-63": {
      title: "Berry Danish Square",
      imageName: "Berry Danish Square"
    },
    "village-pantry-measuring-spoons-63": {
      title: "Measuring Spoons",
      imageName: "Measuring Spoons"
    },
    "village-pantry-jam-label-jar-64": {
      title: "Jam Label Jar",
      imageName: "Jam Label Jar"
    },
    "bakery-window-almond-pinwheel-64": {
      title: "Almond Pinwheel",
      imageName: "Almond Pinwheel"
    },
    "bakery-window-cherry-turnover-65": {
      title: "Cherry Turnover",
      imageName: "Cherry Turnover"
    },
    "village-pantry-tea-strainer-65": {
      title: "Tea Strainer",
      imageName: "Tea Strainer"
    },
    "village-pantry-blue-gingham-cloth-66": {
      title: "Blue Gingham Cloth",
      imageName: "Blue Gingham Cloth"
    },
    "bakery-window-custard-star-66": {
      title: "Custard Star",
      imageName: "Custard Star"
    },
    "bakery-window-poppy-seed-roll-67": {
      title: "Poppy Seed Roll",
      imageName: "Poppy Seed Roll"
    },
    "village-pantry-scalloped-plate-67": {
      title: "Scalloped Plate",
      imageName: "Scalloped Plate"
    },
    "village-pantry-honey-clothespin-68": {
      title: "Honey Clothespin",
      imageName: "Honey Clothespin"
    },
    "bakery-window-maple-palmier-68": {
      title: "Maple Palmier",
      imageName: "Maple Palmier"
    },
    "bakery-window-fig-tart-square-69": {
      title: "Fig Tart Square",
      imageName: "Fig Tart Square"
    },
    "village-pantry-copper-funnel-69": {
      title: "Copper Funnel",
      imageName: "Copper Funnel"
    },
    "village-pantry-embroidered-napkin-70": {
      title: "Embroidered Napkin",
      imageName: "Embroidered Napkin"
    },
    "bakery-window-orange-brioche-knot-70": {
      title: "Orange Brioche Knot",
      imageName: "Orange Brioche Knot"
    },
    "bakery-window-cream-horn-71": {
      title: "Cream Horn",
      imageName: "Cream Horn"
    },
    "village-pantry-linen-bread-bag-71": {
      title: "Linen Bread Bag",
      imageName: "Linen Bread Bag"
    },
    "village-pantry-porcelain-butter-dish-72": {
      title: "Porcelain Butter Dish",
      imageName: "Porcelain Butter Dish"
    },
    "bakery-window-honey-cruller-ring-72": {
      title: "Honey Cruller Ring",
      imageName: "Honey Cruller Ring"
    },
    "bakery-window-raspberry-linzer-frame-73": {
      title: "Raspberry Linzer Frame",
      imageName: "Raspberry Linzer Frame"
    },
    "village-pantry-ceramic-measuring-cup-73": {
      title: "Ceramic Measuring Cup",
      imageName: "Ceramic Measuring Cup"
    },
    "village-pantry-herb-drying-rack-74": {
      title: "Herb Drying Rack",
      imageName: "Herb Drying Rack"
    },
    "bakery-window-blueberry-babka-slice-74": {
      title: "Blueberry Babka Slice",
      imageName: "Blueberry Babka Slice"
    },
    "bakery-window-vanilla-canele-tower-75": {
      title: "Vanilla Canele Tower",
      imageName: "Vanilla Canele Tower"
    },
    "village-pantry-polka-dot-sugar-tin-75": {
      title: "Polka Dot Sugar Tin",
      imageName: "Polka Dot Sugar Tin"
    },
    "village-pantry-wooden-egg-crate-76": {
      title: "Wooden Egg Crate",
      imageName: "Wooden Egg Crate"
    },
    "bakery-window-strawberry-charlotte-dome-76": {
      title: "Strawberry Charlotte Dome",
      imageName: "Strawberry Charlotte Dome"
    },
    "bakery-window-cocoa-biscotti-bundle-77": {
      title: "Cocoa Biscotti Bundle",
      imageName: "Cocoa Biscotti Bundle"
    },
    "village-pantry-checkered-tea-towel-77": {
      title: "Checkered Tea Towel",
      imageName: "Checkered Tea Towel"
    },
    "village-pantry-honeycomb-glass-jar-78": {
      title: "Honeycomb Glass Jar",
      imageName: "Honeycomb Glass Jar"
    },
    "bakery-window-almond-croissant-stack-78": {
      title: "Almond Croissant Stack",
      imageName: "Almond Croissant Stack"
    },
    "bakery-window-peach-cream-tartlet-79": {
      title: "Peach Cream Tartlet",
      imageName: "Peach Cream Tartlet"
    },
    "village-pantry-blue-enamel-colander-79": {
      title: "Blue Enamel Colander",
      imageName: "Blue Enamel Colander"
    },
    "village-pantry-cinnamon-stick-jar-80": {
      title: "Cinnamon Stick Jar",
      imageName: "Cinnamon Stick Jar"
    },
    "bakery-window-lavender-shortbread-tin-80": {
      title: "Lavender Shortbread Tin",
      imageName: "Lavender Shortbread Tin"
    },
    "bakery-window-maple-pecan-braid-81": {
      title: "Maple Pecan Braid",
      imageName: "Maple Pecan Braid"
    },
    "village-pantry-red-check-apron-81": {
      title: "Red Check Apron",
      imageName: "Red Check Apron"
    },
    "village-pantry-pearl-sugar-bowl-82": {
      title: "Pearl Sugar Bowl",
      imageName: "Pearl Sugar Bowl"
    },
    "bakery-window-apricot-jam-tart-82": {
      title: "Apricot Jam Tart",
      imageName: "Apricot Jam Tart"
    },
    "bakery-window-cocoa-swirl-meringue-83": {
      title: "Cocoa Swirl Meringue",
      imageName: "Cocoa Swirl Meringue"
    },
    "village-pantry-gingham-butter-cloche-83": {
      title: "Gingham Butter Cloche",
      imageName: "Gingham Butter Cloche"
    },
    "village-pantry-pressed-flower-frame-84": {
      title: "Pressed Flower Frame",
      imageName: "Pressed Flower Frame"
    },
    "bakery-window-candied-orange-scone-84": {
      title: "Candied Orange Scone",
      imageName: "Candied Orange Scone"
    },
    "bakery-window-rose-cream-eclair-85": {
      title: "Rose Cream Eclair",
      imageName: "Rose Cream Eclair"
    },
    "village-pantry-striped-pickle-jar-85": {
      title: "Striped Pickle Jar",
      imageName: "Striped Pickle Jar"
    },
    "village-pantry-little-recipe-box-86": {
      title: "Little Recipe Box",
      imageName: "Little Recipe Box"
    },
    "bakery-window-hazelnut-praline-square-86": {
      title: "Hazelnut Praline Square",
      imageName: "Hazelnut Praline Square"
    },
    "bakery-window-lemon-curd-rosette-87": {
      title: "Lemon Curd Rosette",
      imageName: "Lemon Curd Rosette"
    },
    "village-pantry-cornflower-tea-canister-87": {
      title: "Cornflower Tea Canister",
      imageName: "Cornflower Tea Canister"
    },
    "village-pantry-ribboned-bread-basket-88": {
      title: "Ribboned Bread Basket",
      imageName: "Ribboned Bread Basket"
    },
    "bakery-window-vanilla-bean-cupcake-88": {
      title: "Vanilla Bean Cupcake",
      imageName: "Vanilla Bean Cupcake"
    },
    "bakery-window-pistachio-glaze-donut-89": {
      title: "Pistachio Glaze Donut",
      imageName: "Pistachio Glaze Donut"
    },
    "village-pantry-sage-thread-spool-89": {
      title: "Sage Thread Spool",
      imageName: "Sage Thread Spool"
    },
    "village-pantry-ceramic-honey-spoon-rest-90": {
      title: "Ceramic Honey Spoon Rest",
      imageName: "Ceramic Honey Spoon Rest"
    },
    "bakery-window-caramel-pear-muffin-90": {
      title: "Caramel Pear Muffin",
      imageName: "Caramel Pear Muffin"
    },
    "bakery-window-sugar-dusted-bundt-91": {
      title: "Sugar Dusted Bundt",
      imageName: "Sugar Dusted Bundt"
    },
    "village-pantry-daisy-milk-bottle-91": {
      title: "Daisy Milk Bottle",
      imageName: "Daisy Milk Bottle"
    },
    "village-pantry-quilted-pot-mat-92": {
      title: "Quilted Pot Mat",
      imageName: "Quilted Pot Mat"
    },
    "bakery-window-berry-cream-crown-92": {
      title: "Berry Cream Crown",
      imageName: "Berry Cream Crown"
    },
    "bakery-window-cocoa-almond-biscuit-93": {
      title: "Cocoa Almond Biscuit",
      imageName: "Cocoa Almond Biscuit"
    },
    "village-pantry-lace-jam-spoon-93": {
      title: "Lace Jam Spoon",
      imageName: "Lace Jam Spoon"
    },
    "village-pantry-mint-label-flour-tin-94": {
      title: "Mint Label Flour Tin",
      imageName: "Mint Label Flour Tin"
    },
    "bakery-window-cherry-cream-brioche-94": {
      title: "Cherry Cream Brioche",
      imageName: "Cherry Cream Brioche"
    },
    "bakery-window-ginger-honey-madeleine-95": {
      title: "Ginger Honey Madeleine",
      imageName: "Ginger Honey Madeleine"
    },
    "village-pantry-blue-ribbon-mason-jar-95": {
      title: "Blue Ribbon Mason Jar",
      imageName: "Blue Ribbon Mason Jar"
    },
    "village-pantry-floral-rolling-pin-96": {
      title: "Floral Rolling Pin",
      imageName: "Floral Rolling Pin"
    },
    "bakery-window-raspberry-choux-puff-96": {
      title: "Raspberry Choux Puff",
      imageName: "Raspberry Choux Puff"
    },
    "bakery-window-lemon-ribbon-tart-97": {
      title: "Lemon Ribbon Tart",
      imageName: "Lemon Ribbon Tart"
    },
    "village-pantry-little-spice-drawer-97": {
      title: "Little Spice Drawer",
      imageName: "Little Spice Drawer"
    },
    "village-pantry-checkered-napkin-ring-98": {
      title: "Checkered Napkin Ring",
      imageName: "Checkered Napkin Ring"
    },
    "bakery-window-almond-crescent-roll-98": {
      title: "Almond Crescent Roll",
      imageName: "Almond Crescent Roll"
    },
    "bakery-window-peach-custard-square-99": {
      title: "Peach Custard Square",
      imageName: "Peach Custard Square"
    },
    "village-pantry-copper-measuring-cups-99": {
      title: "Copper Measuring Cups",
      imageName: "Copper Measuring Cups"
    },
    "village-pantry-blue-check-sugar-tin-100": {
      title: "Blue Check Sugar Tin",
      imageName: "Blue Check Sugar Tin"
    },
    "bakery-window-strawberry-vanilla-puff-100": {
      title: "Strawberry Vanilla Puff",
      imageName: "Strawberry Vanilla Puff"
    },
    "bakery-window-cinnamon-honey-twist-101": {
      title: "Cinnamon Honey Twist",
      imageName: "Cinnamon Honey Twist"
    },
    "village-pantry-green-label-tea-tin-101": {
      title: "Green Label Tea Tin",
      imageName: "Green Label Tea Tin"
    },
    "village-pantry-little-linen-basket-102": {
      title: "Little Linen Basket",
      imageName: "Little Linen Basket"
    },
    "bakery-window-caramel-fig-danish-102": {
      title: "Caramel Fig Danish",
      imageName: "Caramel Fig Danish"
    },
    "bakery-window-blueberry-cream-pinwheel-103": {
      title: "Blueberry Cream Pinwheel",
      imageName: "Blueberry Cream Pinwheel"
    },
    "village-pantry-honey-label-crock-103": {
      title: "Honey Label Crock",
      imageName: "Honey Label Crock"
    },
    "village-pantry-daisy-checked-teapot-104": {
      title: "Daisy Checked Teapot",
      imageName: "Daisy Checked Teapot"
    },
    "bakery-window-plum-cardamom-braid-104": {
      title: "Plum Cardamom Braid",
      imageName: "Plum Cardamom Braid"
    },
    "bakery-window-honey-lavender-canele-105": {
      title: "Honey Lavender Canele",
      imageName: "Honey Lavender Canele"
    },
    "village-pantry-rose-label-jam-pot-105": {
      title: "Rose Label Jam Pot",
      imageName: "Rose Label Jam Pot"
    },
    "village-pantry-blue-linen-bowl-cover-106": {
      title: "Blue Linen Bowl Cover",
      imageName: "Blue Linen Bowl Cover"
    },
    "bakery-window-orange-blossom-cruller-106": {
      title: "Orange Blossom Cruller",
      imageName: "Orange Blossom Cruller"
    },
    "bakery-window-blackberry-vanilla-galette-107": {
      title: "Blackberry Vanilla Galette",
      imageName: "Blackberry Vanilla Galette"
    },
    "village-pantry-gingham-egg-cup-107": {
      title: "Gingham Egg Cup",
      imageName: "Gingham Egg Cup"
    },
    "village-pantry-sage-butter-dish-108": {
      title: "Sage Butter Dish",
      imageName: "Sage Butter Dish"
    },
    "bakery-window-pear-ginger-turnover-108": {
      title: "Pear Ginger Turnover",
      imageName: "Pear Ginger Turnover"
    },
    "bakery-window-mocha-cream-roll-109": {
      title: "Mocha Cream Roll",
      imageName: "Mocha Cream Roll"
    },
    "village-pantry-poppy-seed-mortar-109": {
      title: "Poppy Seed Mortar",
      imageName: "Poppy Seed Mortar"
    },
    "village-pantry-striped-pantry-towel-110": {
      title: "Striped Pantry Towel",
      imageName: "Striped Pantry Towel"
    },
    "bakery-window-cherry-almond-biscotti-110": {
      title: "Cherry Almond Biscotti",
      imageName: "Cherry Almond Biscotti"
    },
    "bakery-window-lemon-poppy-pound-cake-111": {
      title: "Lemon Poppy Pound Cake",
      imageName: "Lemon Poppy Pound Cake"
    },
    "village-pantry-little-cocoa-scoop-111": {
      title: "Little Cocoa Scoop",
      imageName: "Little Cocoa Scoop"
    },
    "village-pantry-sunflower-flour-sieve-112": {
      title: "Sunflower Flour Sieve",
      imageName: "Sunflower Flour Sieve"
    },
    "bakery-window-raspberry-lattice-tart-112": {
      title: "Raspberry Lattice Tart",
      imageName: "Raspberry Lattice Tart"
    },
    "bakery-window-sesame-pretzel-knot-113": {
      title: "Sesame Pretzel Knot",
      imageName: "Sesame Pretzel Knot"
    },
    "village-pantry-porcelain-measuring-jug-113": {
      title: "Porcelain Measuring Jug",
      imageName: "Porcelain Measuring Jug"
    },
    "village-pantry-embroidered-tea-cozy-114": {
      title: "Embroidered Tea Cozy",
      imageName: "Embroidered Tea Cozy"
    },
    "bakery-window-cranberry-linzer-star-114": {
      title: "Cranberry Linzer Star",
      imageName: "Cranberry Linzer Star"
    },
    "bakery-window-hazelnut-cocoa-tart-115": {
      title: "Hazelnut Cocoa Tart",
      imageName: "Hazelnut Cocoa Tart"
    },
    "village-pantry-braided-herb-basket-115": {
      title: "Braided Herb Basket",
      imageName: "Braided Herb Basket"
    },
    "village-pantry-blue-daisy-teacup-116": {
      title: "Blue Daisy Teacup",
      imageName: "Blue Daisy Teacup"
    },
    "bakery-window-pistachio-crescent-wreath-116": {
      title: "Pistachio Crescent Wreath",
      imageName: "Pistachio Crescent Wreath"
    },
    "bakery-window-apricot-custard-bar-117": {
      title: "Apricot Custard Bar",
      imageName: "Apricot Custard Bar"
    },
    "village-pantry-copper-tea-strainer-117": {
      title: "Copper Tea Strainer",
      imageName: "Copper Tea Strainer"
    },
    "village-pantry-gingham-picnic-jar-118": {
      title: "Gingham Picnic Jar",
      imageName: "Gingham Picnic Jar"
    },
    "bakery-window-maple-pecan-scroll-118": {
      title: "Maple Pecan Scroll",
      imageName: "Maple Pecan Scroll"
    },
    "bakery-window-vanilla-checker-tile-119": {
      title: "Vanilla Checker Tile",
      imageName: "Vanilla Checker Tile"
    },
    "village-pantry-wooden-pastry-brush-119": {
      title: "Wooden Pastry Brush",
      imageName: "Wooden Pastry Brush"
    },
    "village-pantry-ceramic-spoon-rest-120": {
      title: "Ceramic Spoon Rest",
      imageName: "Ceramic Spoon Rest"
    },
    "bakery-window-orange-cardamom-ring-120": {
      title: "Orange Cardamom Ring",
      imageName: "Orange Cardamom Ring"
    },
    "bakery-window-berry-cream-envelope-121": {
      title: "Berry Cream Envelope",
      imageName: "Berry Cream Envelope"
    },
    "village-pantry-sage-linen-bundle-121": {
      title: "Sage Linen Bundle",
      imageName: "Sage Linen Bundle"
    },
    "village-pantry-honey-dipper-jar-122": {
      title: "Honey Dipper Jar",
      imageName: "Honey Dipper Jar"
    },
    "bakery-window-cherry-cream-crown-122": {
      title: "Cherry Cream Crown",
      imageName: "Cherry Cream Crown"
    },
    "bakery-window-lemon-zest-petal-tart-123": {
      title: "Lemon Zest Petal Tart",
      imageName: "Lemon Zest Petal Tart"
    },
    "village-pantry-striped-sugar-sack-123": {
      title: "Striped Sugar Sack",
      imageName: "Striped Sugar Sack"
    },
    "village-pantry-tiny-copper-kettle-124": {
      title: "Tiny Copper Kettle",
      imageName: "Tiny Copper Kettle"
    },
    "bakery-window-fig-honey-pinwheel-124": {
      title: "Fig Honey Pinwheel",
      imageName: "Fig Honey Pinwheel"
    },
    "bakery-window-cocoa-pear-tartlet-125": {
      title: "Cocoa Pear Tartlet",
      imageName: "Cocoa Pear Tartlet"
    },
    "village-pantry-checked-recipe-folder-125": {
      title: "Checked Recipe Folder",
      imageName: "Checked Recipe Folder"
    },
    "village-pantry-berry-label-sifter-126": {
      title: "Berry Label Sifter",
      imageName: "Berry Label Sifter"
    }
,
    "bakery-window-apple-cinnamon-rose-126": {
      title: "Apple Cinnamon Rose",
      imageName: "Apple Cinnamon Rose"
    },
    "bakery-window-honey-sesame-twist-127": {
      title: "Honey Sesame Twist",
      imageName: "Honey Sesame Twist"
    },
    "village-pantry-blue-stripe-flour-crock-127": {
      title: "Blue Stripe Flour Crock",
      imageName: "Blue Stripe Flour Crock"
    },
    "village-pantry-copper-berry-scoop-128": {
      title: "Copper Berry Scoop",
      imageName: "Copper Berry Scoop"
    },
    "bakery-window-plum-cream-danish-128": {
      title: "Plum Cream Danish",
      imageName: "Plum Cream Danish"
    },
    "bakery-window-ginger-caramel-knot-129": {
      title: "Ginger Caramel Knot",
      imageName: "Ginger Caramel Knot"
    },
    "village-pantry-mint-label-tea-tin-129": {
      title: "Mint Label Tea Tin",
      imageName: "Mint Label Tea Tin"
    },
    "village-pantry-stitched-linen-apron-130": {
      title: "Stitched Linen Apron",
      imageName: "Stitched Linen Apron"
    },
    "bakery-window-cranberry-custard-braid-130": {
      title: "Cranberry Custard Braid",
      imageName: "Cranberry Custard Braid"
    },
    "bakery-window-mocha-hazelnut-button-131": {
      title: "Mocha Hazelnut Button",
      imageName: "Mocha Hazelnut Button"
    },
    "village-pantry-sunflower-spice-tin-131": {
      title: "Sunflower Spice Tin",
      imageName: "Sunflower Spice Tin"
    },
    "village-pantry-gingham-bread-cloth-132": {
      title: "Gingham Bread Cloth",
      imageName: "Gingham Bread Cloth"
    },
    "bakery-window-pear-vanilla-rosette-132": {
      title: "Pear Vanilla Rosette",
      imageName: "Pear Vanilla Rosette"
    },
    "bakery-window-blueberry-almond-square-133": {
      title: "Blueberry Almond Square",
      imageName: "Blueberry Almond Square"
    },
    "bakery-window-apricot-sugar-shell-134": {
      title: "Apricot Sugar Shell",
      imageName: "Apricot Sugar Shell"
    },
    "bakery-window-cocoa-cherry-ribbon-135": {
      title: "Cocoa Cherry Ribbon",
      imageName: "Cocoa Cherry Ribbon"
    },
    "bakery-window-lemon-thyme-crown-136": {
      title: "Lemon Thyme Crown",
      imageName: "Lemon Thyme Crown"
    },
    "village-pantry-patchwork-jam-ledger-133": {
      title: "Patchwork Jam Ledger",
      imageName: "Patchwork Jam Ledger"
    },
    "village-pantry-sage-butter-crock-134": {
      title: "Sage Butter Crock",
      imageName: "Sage Butter Crock"
    },
    "village-pantry-copper-honey-measure-135": {
      title: "Copper Honey Measure",
      imageName: "Copper Honey Measure"
    },
    "village-pantry-daisy-recipe-clipboard-136": {
      title: "Daisy Recipe Clipboard",
      imageName: "Daisy Recipe Clipboard"
    },
    "village-pantry-tiny-checkered-sieve-137": {
      title: "Tiny Checkered Sieve",
      imageName: "Tiny Checkered Sieve"
    }

  }
};
