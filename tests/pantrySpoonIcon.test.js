...
describe("Pantry spoon currency rendering", () => {
  ...
  it("provides one shared balance chip across every view", () => {
    ...
    expect(stylesSource).toMatch(/\.app-shell--workshop-home \.spoon-balance-chip\s*\{[\s\S]*?right:\s*max\(132px, calc\(env\(safe-area-inset-right, 0px\) \+ 132px\)\);[\s\S]*?background:\s*transparent;/);
