...
describe("puzzle data", () => {
  ...
  it("keeps larger boards limited to larger-board progression packs", () => {
    ...
    expect(progressionPuzzles.filter((puzzle) => puzzle.size >= 12).length).toBeGreaterThanOrEqual(51);
    progressionPuzzles.forEach((puzzle) => {
      ...
      if (puzzle.packId === "bakery-window") {
        expect(progressionPuzzles.filter((candidate) => candidate.packId === "bakery-window" && candidate.size === 12).length).toBeGreaterThanOrEqual(51);
      }
      if (puzzle.size > 8) {
  ...
  it("keeps large-board translated catalog metadata explicit", () => {
    ...
    expect(largeBoardPuzzles.length).toBeGreaterThanOrEqual(113);
  ...
  it("keeps recent readable silhouettes off fully blank board edges", () => {
    ...
    expect(recentReadablePuzzles.length).toBeGreaterThanOrEqual(49);
